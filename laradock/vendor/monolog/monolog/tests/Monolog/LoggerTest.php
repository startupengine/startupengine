...
class LoggerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function useMicrosecondTimestampsProvider()
    {
        return array(
            ...
            'without microseconds' => array(
                false,
                PHP_VERSION_ID >= 70100 ? 'assertNotSame' : 'assertSame'
            )
        );
    }
    ...
    public function testDefaultHandleException()
    {
        ...
        $handler
            ...
            ->method('handle')
            ->will(
                $this->throwException(new \Exception('Some handler exception'))
            );
        $logger->pushHandler($handler);
        $logger->info('test');
    }
    ...
    public function testReset()
    {
        ...
        $fingersCrossedHandler = new Handler\FingersCrossedHandler(
            $groupHandler
        );
        ...
        $that = $this;
        $assertBufferOfBufferHandlerEmpty = function () use (
            $getProperty,
            $bufferHandler,
            $that
        ) {
        ...
        $assertBuffersEmpty = function () use (
