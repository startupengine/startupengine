...
class MongoDBFormatterTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        return array(array(1, true, 1, true), array(0, false, 0, false));
    }
    ...
    public function testConstruct(
        ...
        $reflTrace = new \ReflectionProperty(
            $formatter,
    ...
    public function testSimpleFormat()
    {
        ...
        $this->assertEquals(
            ...
            $formattedRecord['level_name']
        );
        $this->assertEquals('test', $formattedRecord['channel']);
        $this->assertInstanceOf('\MongoDate', $formattedRecord['datetime']);
        $this->assertEquals(
            '0.00000000 1391212800',
            $formattedRecord['datetime']->__toString()
        );
    ...
    public function testRecursiveFormat()
    {
        ...
        $this->assertInstanceOf(
            '\MongoDate',
            $formattedRecord['context']['stuff']
        );
        $this->assertEquals(
            '0.00000000 1391221893',
            $formattedRecord['context']['stuff']->__toString()
        );
        $this->assertEquals(
            array(
                'foo' => 'something',
        ...
        $this->assertEquals(
            'some string',
            $formattedRecord['context']['context_string']
        );
        ...
        $this->assertEquals(
            'exception message',
            $formattedRecord['context']['except']['message']
        );
        $this->assertEquals(987, $formattedRecord['context']['except']['code']);
        $this->assertInternalType(
            'string',
            $formattedRecord['context']['except']['file']
        );
        $this->assertInternalType(
            'integer',
            $formattedRecord['context']['except']['code']
        );
        $this->assertInternalType(
            'string',
            $formattedRecord['context']['except']['trace']
        );
        $this->assertEquals(
            'Exception',
            $formattedRecord['context']['except']['class']
        );
    }
    ...
    public function testFormatDepthException()
    {
        ...
        $this->assertEquals(
            'exception message',
            $formattedRecord['context']['nest2']['message']
        );
        $this->assertEquals(987, $formattedRecord['context']['nest2']['code']);
        $this->assertEquals(
            '[...]',
            $formattedRecord['context']['nest2']['trace']
        );
    }
}
