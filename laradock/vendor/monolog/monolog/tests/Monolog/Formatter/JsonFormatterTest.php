...
class JsonFormatterTest extends TestCase
{
    ...
    public function testDefFormatWithException()
    {
        ...
        $message = $this->formatRecordWithExceptionInContext(
            $formatter,
            $exception
        );

        $this->assertContextContainsFormattedException(
            $formattedException,
            $message
        );
    }
    ...
    public function testDefFormatWithPreviousException()
    {
        ...
        $exception = new \RuntimeException(
            'Foo',
            0,
            new \LogicException('Wut?')
        );
        $formattedPrevException = $this->formatException(
            $exception->getPrevious()
        );
        $formattedException = $this->formatException(
            $exception,
            $formattedPrevException
        );
    ...
    {
        $this->assertEquals(
            '{"level_name":"CRITICAL","channel":"core","context":{"exception":' .
                $expected .
     ...
     * @return string
     */
    private function formatRecordWithExceptionInContext(
    ...
    private function formatException($exception, $previous = null)
    {
        $formattedException =
            ...
            $exception->getCode() .
            ',"file":"' .
            $this->formatExceptionFilePathWithLine($exception) .
