...
class RavenHandlerTest extends TestCase
{
    ...
    public function testTag()
    {
        ...
        $tags = array(1, 2, 'foo');
        $record = $this->getRecord(Logger::INFO, 'test', array(
            'tags' => $tags
        ));
        $handler->handle($record);
    ...
    public function testFingerprint()
    {
        ...
        $this->assertEquals(
            $fingerprint,
            $ravenClient->lastData['fingerprint']
        );
    }
    ...
    public function testUserContext()
    {
        ...
        $recordWithNoContext = $this->getRecord(
            Logger::INFO,
            'test with default user context'
        );
        ...
        $recordWithContext = $this->getRecord(Logger::INFO, 'test', array(
            'user' => $user
        ));
    ...
    public function testException()
    {
        ...
        try {
            ...
            $record = $this->getRecord(Logger::ERROR, $e->getMessage(), array(
                'exception' => $e
            ));
            $handler->handle($record);
        }
    ...
    public function testHandleBatchDoNothingIfRecordsAreBelowLevel()
    {
        ...
        $handler = $this->getMock('Monolog\Handler\RavenHandler', null, array(
            $this->getRavenClient()
        ));
    ...
    public function testHandleBatchPicksProperMessage()
    {
        ...
        $formatter
            ...
            ->method('format')
            ->with(
                $this->callback(function ($record) use ($records) {
