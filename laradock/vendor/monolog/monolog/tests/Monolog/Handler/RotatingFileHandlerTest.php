...
class RotatingFileHandlerTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        if (!is_writable($dir)) {
            $this->markTestSkipped(
                $dir . ' must be writable to test the RotatingFileHandler.'
            );
        }
        $this->lastError = null;
        $self = $this;
    ...
    {
        touch(
            __DIR__ . '/Fixtures/foo-' . date('Y-m-d', time() - 86400) . '.rot'
        );
    ...
    public function testRotation($createFile, $dateFormat, $timeCallback)
    {
        touch(
            ($old1 =
                __DIR__ .
                '/Fixtures/foo-' .
                date($dateFormat, $timeCallback(-1)) .
                '.rot')
        );
        touch(
            ($old2 =
                __DIR__ .
                '/Fixtures/foo-' .
                date($dateFormat, $timeCallback(-2)) .
                '.rot')
        );
        touch(
            ($old3 =
                __DIR__ .
                '/Fixtures/foo-' .
                date($dateFormat, $timeCallback(-3)) .
                '.rot')
        );
        touch(
            ($old4 =
                __DIR__ .
                '/Fixtures/foo-' .
                date($dateFormat, $timeCallback(-4)) .
                '.rot')
        );
    ...
    public function rotationTests()
    {
        ...
        return array(
            'Rotation is triggered when the file of the current day is not present' => array(
                true,
                RotatingFileHandler::FILE_PER_DAY,
                $dayCallback
            ),
            'Rotation is not triggered when the file of the current day is already present' => array(
                false,
                RotatingFileHandler::FILE_PER_DAY,
                $dayCallback
            ),

            'Rotation is triggered when the file of the current month is not present' => array(
                true,
                RotatingFileHandler::FILE_PER_MONTH,
                $monthCallback
            ),
            'Rotation is not triggered when the file of the current month is already present' => array(
                false,
                RotatingFileHandler::FILE_PER_MONTH,
                $monthCallback
            ),

            'Rotation is triggered when the file of the current year is not present' => array(
                true,
                RotatingFileHandler::FILE_PER_YEAR,
                $yearCallback
            ),
            'Rotation is not triggered when the file of the current year is already present' => array(
                false,
                RotatingFileHandler::FILE_PER_YEAR,
                $yearCallback
            )
     ...
     */
    public function testDisallowFilenameFormatsWithoutDate(
        $filenameFormat,
        $valid
    ) {
        $handler = new RotatingFileHandler(__DIR__ . '/Fixtures/foo.rot', 2);
    ...
    public function testRotationWhenSimilarFileNamesExist($dateFormat)
    {
        touch(
            ($old1 =
                __DIR__ . '/Fixtures/foo-foo-' . date($dateFormat) . '.rot')
        );
        touch(
            ($old2 =
                __DIR__ . '/Fixtures/foo-bar-' . date($dateFormat) . '.rot')
        );
    ...
    {
        return array(
            'Rotation is triggered when the file of the current day is not present but similar exists' => array(
                RotatingFileHandler::FILE_PER_DAY
            ),

            'Rotation is triggered when the file of the current month is not present but similar exists' => array(
                RotatingFileHandler::FILE_PER_MONTH
            ),

            'Rotation is triggered when the file of the current year is not present but similar exists' => array(
                RotatingFileHandler::FILE_PER_YEAR
            )
        );
    }
