...
class SlackRecordTest extends TestCase
{
    ...
    {
        $this->jsonPrettyPrintFlag = defined('JSON_PRETTY_PRINT')
            ? JSON_PRETTY_PRINT
            : 128;
    }
    ...
    {
        $jsonPrettyPrintFlag = defined('JSON_PRETTY_PRINT')
            ? JSON_PRETTY_PRINT
            : 128;
        ...
        return array(
            ...
            array(
                $multipleDimensions,
                json_encode($multipleDimensions, $jsonPrettyPrintFlag)
            ),
            array(
                $numericKeys,
                json_encode($numericKeys, $jsonPrettyPrintFlag)
            ),
    ...
    public function testTextEqualsMessageIfNoAttachment()
    {
        ...
        $record = new SlackRecord(null, null, false);
        $data = $record->getSlackData(
            $this->getRecord(Logger::WARNING, $message)
        );
    ...
    public function testMapsLevelToColorAttachmentColor()
    {
        ...
        $this->assertSame(
            SlackRecord::COLOR_DANGER,
            $data['attachments'][0]['color']
        );
    ...
    public function testAddsShortAttachmentWithoutContextAndExtra()
    {
        ...
        $data = $record->getSlackData(
            $this->getRecord($level, 'test', array('test' => 1))
        );
        ...
        $this->assertSame(
            array(
                array(
                    ...
                    'value' => sprintf(
                        '```%s```',
                        json_encode($extra, $this->jsonPrettyPrintFlag)
                    ),
                    'short' => false
                ),
                array(
                    ...
                    'value' => sprintf(
                        '```%s```',
                        json_encode($context, $this->jsonPrettyPrintFlag)
                    ),
                    'short' => false
                )
            ),
    ...
    public function testContextHasException()
    {
        ...
        $this->assertInternalType(
            'string',
            $data['attachments'][0]['fields'][1]['value']
        );
    }
    ...
    public function testExcludeExtraAndContextFields()
    {
        $record = $this->getRecord(Logger::WARNING, 'test', array(
            'info' => array('library' => 'monolog', 'author' => 'Jordi')
        ));
        ...
        $slackRecord = new SlackRecord(
            ...
            false,
            true,
            array('context.info.library', 'extra.tags.1')
        );
        $data = $slackRecord->getSlackData($record);
        $attachment = $data['attachments'][0];
