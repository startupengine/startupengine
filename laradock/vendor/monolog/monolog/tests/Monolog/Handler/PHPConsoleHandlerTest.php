...
class PHPConsoleHandlerTest extends TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->debugDispatcher = $this->initDebugDispatcherMock(
            $this->connector
        );
        $this->connector->setDebugDispatcher($this->debugDispatcher);

        $this->errorDispatcher = $this->initErrorDispatcherMock(
            $this->connector
        );
    ...
    public function testInitWithDefaultConnector()
    {
        ...
        $this->assertEquals(
            spl_object_hash(Connector::getInstance()),
            spl_object_hash($handler->getConnector())
        );
    }
    ...
    public function testInitWithCustomConnector()
    {
        $handler = new PHPConsoleHandler(array(), $this->connector);
        $this->assertEquals(
            spl_object_hash($this->connector),
            spl_object_hash($handler->getConnector())
        );
    }
    ...
    public function testDebugContextInMessage()
    {
        ...
        $expectedMessage =
            $message . ' ' . json_encode(array_slice($context, 1));
        $this->debugDispatcher
            ->expects($this->once())
            ->method('dispatchDebug')
            ->with($this->equalTo($expectedMessage), $this->equalTo($tag));
    ...
    public function testDebugTags($tagsContextKeys = null)
    {
        ...
        $logger = $this->initLogger(
            $tagsContextKeys
                ? array('debugTagsKeysInContext' => $tagsContextKeys)
                : array()
        );
        if (!$tagsContextKeys) {
            $tagsContextKeys = $this->getHandlerDefaultOption(
        ...
        foreach ($tagsContextKeys as $key) {
            ...
            $debugDispatcher
                ->expects($this->once())
                ->method('dispatchDebug')
                ->with($this->anything(), $this->equalTo($expectedTags));
    ...
    public function testError($classesPartialsTraceIgnore = null)
    {
        ...
        $this->errorDispatcher
            ...
            ->with(
                ...
                $this->equalTo(
                    $this->getHandlerDefaultOption('classesPartialsTraceIgnore')
                )
            );
        $errorHandler = ErrorHandler::register(
            $this->initLogger(
                $classesPartialsTraceIgnore
                    ? array(
                        'classesPartialsTraceIgnore' => $classesPartialsTraceIgnore
                    )
                    : array()
            ),
            false
    ...
    {
        $this->initLogger(array(
            'useOwnErrorsHandler' => true,
            'useOwnExceptionsHandler' => true
        ));
        $this->assertEquals(
            array(Handler::getInstance(), 'handleError'),
            set_error_handler(function () {})
        );
        $this->assertEquals(
            array(Handler::getInstance(), 'handleException'),
     ...
     */
    public function testOptionCallsConnectorMethod(
        $option,
        $method,
        $value,
        $isArgument = true
    ...
    public function testOptionDetectDumpTraceAndSource()
    {
        new PHPConsoleHandler(
            array('detectDumpTraceAndSource' => true),
            $this->connector
        );
        $this->assertTrue(
            $this->connector->getDebugDispatcher()->detectTraceAndSource
        );
    }
    ...
    public function testDumperOptions($option, $dumperProperty, $value)
    {
        ...
        $this->assertEquals(
            $value,
            $this->connector->getDumper()->$dumperProperty
        );
    }
}
