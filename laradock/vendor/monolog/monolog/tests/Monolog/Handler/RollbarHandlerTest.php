...
class RollbarHandlerTest extends TestCase
{
    ...
    public function testExceptionLogLevel()
    {
        ...
        $this->assertEquals(
            'debug',
            $this->reportedExceptionArguments['payload']['level']
        );
    }
    ...
    private function setupRollbarNotifierMock()
    {
        ...
        $this->rollbarNotifier
            ...
            ->willReturnCallback(function ($exception, $context, $payload) use (
                $that
            ) {
                $that->reportedExceptionArguments = compact(
                    'exception',
                    'context',
                    'payload'
                );
    ...
    private function createExceptionRecord(
        $level = Logger::DEBUG,
        $message = 'test',
        $exception = null
    ) {
