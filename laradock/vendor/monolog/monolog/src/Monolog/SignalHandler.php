...
class SignalHandler
{
    ...
    public function handleSignal($signo, array $siginfo = null)
    {
        ...
        $level = isset($this->signalLevelMap[$signo])
            ? $this->signalLevelMap[$signo]
            : LogLevel::CRITICAL;
        $signal = isset($signals[$signo]) ? $signals[$signo] : $signo;
        $context = isset($siginfo) ? $siginfo : array();
        $this->logger->log(
            $level,
            sprintf('Program received signal %s', $signal),
            $context
        );
        ...
        if (
            $this->previousSignalHandler[$signo] === true ||
            $this->previousSignalHandler[$signo] === SIG_DFL
        ) {
            if (
                ...
                $restartSyscalls = isset($this->restartSyscalls[$signo])
                    ? $this->restartSyscalls[$signo]
                    : true;
