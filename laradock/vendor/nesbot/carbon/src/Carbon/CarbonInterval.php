...
class CarbonInterval extends DateInterval
{
    ...
    public static function getCascadeFactors()
    {
        return static::$cascadeFactors ?: [
                ...
                'months' => [
                    Carbon::DAYS_PER_WEEK * Carbon::WEEKS_PER_MONTH,
                    'dayz'
                ],
    ...
    private static function getFlipCascadeFactors()
    {
        if (!self::$flipCascadeFactors) {
            ...
            foreach (static::getCascadeFactors() as $to => [$factor, $from]) {
                self::$flipCascadeFactors[self::standardizeUnit($from)] = [
                    self::standardizeUnit($to),
                    $factor
                ];
            }
        }
    ...
    {
        return static::getFactor('minutes', 'hours') ?:
    ...
    {
        return static::getFactor('seconds', 'minutes') ?:
    ...
    {
        return static::getFactor('milliseconds', 'seconds') ?:
            Carbon::MILLISECONDS_PER_SECOND;
    }
    ...
    {
        return static::getFactor('microseconds', 'milliseconds') ?:
            Carbon::MICROSECONDS_PER_MILLISECOND;
    }
    ...
    public static function __callStatic($method, $parameters)
    {
        ...
        switch (Carbon::singularUnit(rtrim($method, 'z'))) {
            ...
            case 'milli':
                return new static(
                    ...
                    null,
                    null,
                    $arg * Carbon::MICROSECONDS_PER_MILLISECOND
                );
        ...
        if (Carbon::isStrictModeEnabled()) {
            throw new BadMethodCallException(
                sprintf("Unknown fluent constructor '%s'.", $method)
            );
        }
            ...
            switch ($unit === 'µs' ? 'µs' : strtolower($unit)) {
                ...
                case 'w':
                    ...
                    if ($fraction) {
                        $parts[] = [
                            null,
                            $fraction * static::getDaysPerWeek(),
                            'd'
                        ];
                    }
                    break;
                ...
                case 'd':
                    ...
                    if ($fraction) {
                        $parts[] = [
                            null,
                            $fraction * static::getHoursPerDay(),
                            'h'
                        ];
                    }
                    break;
                ...
                case 'h':
                    ...
                    if ($fraction) {
                        $parts[] = [
                            null,
                            $fraction * static::getMinutesPerHour(),
                            'm'
                        ];
                    }
                    break;
                ...
                case 'm':
                    ...
                    if ($fraction) {
                        $parts[] = [
                            null,
                            $fraction * static::getSecondsPerMinute(),
                            's'
                        ];
                    }
                    break;
                ...
                case 's':
                    ...
                    if ($fraction) {
                        $parts[] = [
                            null,
                            $fraction * static::getMillisecondsPerSecond(),
                            'ms'
                        ];
                    }
                    break;
                ...
                case 'ms':
                    ...
                    if ($fraction) {
                        $microseconds += round(
                            $fraction * static::getMicrosecondsPerMillisecond()
                        );
                    }
                    break;
    ...
    public function __get($name)
    {
        ...
        switch ($name) {
            ...
            case 'milliseconds':
                return (int) floor(
                    round($this->f * Carbon::MICROSECONDS_PER_SECOND) /
                        Carbon::MICROSECONDS_PER_MILLISECOND
                );
    ...
    public function __set($name, $value)
    {
        switch (Carbon::singularUnit(rtrim($name, 'z'))) {
            ...
            default:
                if (
                    $this->localStrictModeEnabled ??
                    Carbon::isStrictModeEnabled()
                ) {
                    throw new InvalidArgumentException(
                        sprintf("Unknown setter '%s'", $name)
                    );
                }
        ...
        switch (Carbon::singularUnit(rtrim($method, 'z'))) {
            ...
            default:
                if (
                    ...
                    throw new BadMethodCallException(
                        sprintf("Unknown fluent setter '%s'", $method)
                    );
                }
        }
    ...
    protected function getForHumansParameters(
        $syntax = null,
        $short = false,
        $parts = -1,
        $options = null
        ...
        if ($join === true) {
            $default =
                $this->getTranslationMessage('list.0') ??
                $this->getTranslationMessage('list') ??
                ' ';
            $join = [
                $default,
    ...
    public function forHumans(
        ...
        [
            ...
            $join,
            $aUnit
        ] = $this->getForHumansParameters($syntax, $short, $parts, $options);

        $interval = [];
        $syntax = (int) $syntax === null
        ...
        foreach ($diffIntervalArray as $diffIntervalData) {
            if ($diffIntervalData['value'] > 0) {
                $unit = $short
                    ? $diffIntervalData['unitShort']
                    : $diffIntervalData['unit'];
    ...
    public function add($unit, $value = 1)
    {
        ...
        $this->dayz +=
            ($interval->days === false ? $interval->d : $interval->days) *
            $sign;
    ...
    {
        foreach (
            static::getFlipCascadeFactors()
            as $source => [$target, $factor]
        ) {
