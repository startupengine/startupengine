...
class CarbonPeriod implements Iterator, Countable
{
    ...
    protected static function parseIso8601($iso)
    {
        ...
        foreach (explode('/', $iso) as $key => $part) {
            if ($key === 0 && preg_match('/^R([0-9]*)$/', $part, $match)) {
                ...
                $interval === null &&
                ($parsed = CarbonInterval::make($part))
            ) {
                $interval = $part;
                ...
                $end === null &&
                ($parsed = Carbon::make(static::addMissingParts($start, $part)))
            ) {
                $end = $part;
            } else {
    ...
    protected static function addMissingParts($source, $target)
    {
        $pattern =
            '/' .
            preg_replace('/[0-9]+/', '[0-9]+', preg_quote($target, '/')) .
            '$/';
    ...
    protected function isCarbonPredicateMethod($callable)
    {
        return is_string($callable) &&
            ...
            (method_exists($this->dateClass, $callable) ||
                call_user_func([$this->dateClass, 'hasMacro'], $callable));
    }
    ...
    public function toggleOptions($options, $state = null)
    {
        ...
        return $this->setOptions(
            $state ? $this->options | $options : $this->options & ~$options
        );
    }
    ...
    protected function createFilterTuple(array $parameters)
    {
        ...
        return [
            function ($date) use ($method, $parameters) {
    ...
    public function removeFilter($filter)
    {
        ...
        $this->filters = array_values(
            array_filter($this->filters, function ($tuple) use ($key, $filter) {
    ...
    {
        if (
            (!is_numeric($recurrences) && !is_null($recurrences)) ||
            $recurrences < 0
        ) {
            throw new InvalidArgumentException(
    ...
    {
        if (
            $this->getOptions() & static::IMMUTABLE &&
    ...
    public function rewind()
    {
        ...
        $this->timezone = static::intervalHasTime($this->dateInterval)
            ? $this->current->getTimezone()
            : null;
    ...
    public function toString()
    {
        ...
        $format =
            !$this->startDate->isStartOfDay() ||
            ($this->endDate && !$this->endDate->isStartOfDay())
                ? 'Y-m-d H:i:s'
                : 'Y-m-d';
        ...
        $parts[] = $this->translate(
            'period_interval',
            [
                ':interval' => $this->dateInterval->forHumans([
                    'join' => true
                ])
            ],
            null,
        ...
        $parts[] = $this->translate(
            'period_start_date',
            [':date' => $this->startDate->rawFormat($format)],
            null,
            $translator
        );
        ...
        if ($this->endDate !== null) {
            $parts[] = $this->translate(
                'period_end_date',
                [':date' => $this->endDate->rawFormat($format)],
                null,
                $translator
            );
        }
