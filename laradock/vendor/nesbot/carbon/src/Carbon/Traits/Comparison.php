...
trait Comparison
{
    ...
    public function between($date1, $date2, $equal = true): bool
    {
        ...
        if ($equal) {
            return $this->greaterThanOrEqualTo($date1) &&
                $this->lessThanOrEqualTo($date2);
        }
    ...
    public function isYesterday()
    {
        return $this->toDateString() ===
            static::yesterday($this->getTimezone())->toDateString();
    }
    ...
    {
        return $this->toDateString() ===
            static::tomorrow($this->getTimezone())->toDateString();
    }
    ...
    {
        return static::create($this->year, 12, 28, 0, 0, 0, $this->tz)
            ->weekOfYear === 53;
    }
    ...
    public function isSameAs($format, $date = null)
    {
        ...
        return $this->rawFormat($format) ===
            ($date instanceof self
                ? $date->rawFormat($format)
                : $date->format($format));
    }
    ...
    public function isSameUnit($unit, $date = null)
    {
        ...
        if (!isset($units[$unit])) {
            ...
            if (
                $this->localStrictModeEnabled ?? static::isStrictModeEnabled()
            ) {
    ...
    public function isSameQuarter($date = null, $ofSameYear = true)
    {
        ...
        return $this->quarter === $date->quarter &&
            (!$ofSameYear || $this->isSameYear($date));
    }
    ...
    public function isDayOfWeek($dayOfWeek)
    {
        if (
            is_string($dayOfWeek) &&
            defined(($constant = static::class . '::' . strtoupper($dayOfWeek)))
        ) {
