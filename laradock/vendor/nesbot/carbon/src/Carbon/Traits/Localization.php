...
trait Localization
{
    ...
    {
        static::$humanDiffOptions =
            static::getHumanDiffOptions() | $humanDiffOption;
    }
     ...
     * @return string
     */
    public static function getTranslationMessageWith(
        $translator,
        string $key,
        string $locale = null,
        string $default = null
        ...
        $result = $translator
            ->getCatalogue($locale ?? $translator->getLocale())
            ->get($key);
     ...
     * @return string
     */
    public static function translateWith(
        TranslatorInterface $translator,
        string $key,
        array $parameters = [],
        $number = null
    ): string {
        $message = static::getTranslationMessageWith(
            $translator,
            $key,
            null,
            $key
    ...
    public static function translateTimeString(
        ...
        $from = null,
        $to = null,
        $mode = CarbonInterface::TRANSLATE_ALL
    ) {
        ...
        foreach (['from', 'to'] as $key) {
                ...
                foreach (['months', 'weekdays'] as $variable) {
                    ...
                    if ($list) {
                        foreach ($$variable as $index => &$name) {
                            $name .=
                                '|' .
                                $messages[$variable . '_standalone'][$index];
                        }
                    }
                }
            }
            ...
            $$translationKey = array_merge(
                ...
                $mode & CarbonInterface::TRANSLATE_DAYS
                    ? array_pad(
                        $messages['weekdays_short'],
                        7,
                        '>>DO NOT REPLACE<<'
                    )
                    : [],
                $mode & CarbonInterface::TRANSLATE_UNITS
                    ? array_map(
                        function ($unit) use ($messages, $key, $cleanWord) {
                    ...
                    : [],
                $mode & CarbonInterface::TRANSLATE_MERIDIEM
                    ? array_map(function ($hour) use ($meridiem) {
        ...
        return substr(
            preg_replace_callback(
                '/(?<=[\d\s+.\/,_-])(' .
                    implode('|', $fromTranslations) .
                    ')(?=[\d\s+.\/,_-])/i',
                function ($match) use ($fromTranslations, $toTranslations) {
                    [$chunk] = $match;
    ...
    {
        return static::translateTimeString(
            $timeString,
            $this->getLocalTranslator()->getLocale(),
            $to
        );
    }
    ...
    public function locale(string $locale = null, ...$fallbackLocales)
    {
        ...
        if (
            ...
            $this->localTranslator->getLocale() !== $locale
        ) {
    ...
    public static function executeWithLocale($locale, $func)
    {
        ...
        $result = call_user_func(
            $func,
            static::setLocale($locale) ? static::getLocale() : false,
            static::translator()
        );
    ...
    {
        return static::executeWithLocale($locale, function (
            $newLocale,
            TranslatorInterface $translator
        ) {
            return ($newLocale &&
    ...
    public static function localeHasDiffSyntax($locale)
    {
        return static::executeWithLocale($locale, function (
            ...
            foreach (['ago', 'from_now', 'before', 'after'] as $key) {
                if (
                    $translator instanceof TranslatorBagInterface &&
                    $translator->getCatalogue($newLocale)->get($key) instanceof
                        Closure
                ) {
                    continue;
                }
    ...
    public static function localeHasDiffTwoDayWords($locale)
    {
        return static::executeWithLocale($locale, function (
            ...
            return $newLocale &&
                $translator->trans('diff_before_yesterday') !==
                    'diff_before_yesterday' &&
                $translator->trans('diff_after_tomorrow') !==
                    'diff_after_tomorrow';
        });
    }
    ...
    public static function localeHasPeriodSyntax($locale)
    {
        return static::executeWithLocale($locale, function (
            ...
            return $newLocale &&
                $translator->trans('period_recurrences') !==
                    'period_recurrences' &&
                $translator->trans('period_interval') !== 'period_interval' &&
                $translator->trans('period_start_date') !==
                    'period_start_date' &&
