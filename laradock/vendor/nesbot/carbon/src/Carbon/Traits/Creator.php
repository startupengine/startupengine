...
trait Creator
{
    ...
    {
        return static::rawParse(
            static::translateTimeString($time, $locale, 'en'),
            $tz
        );
    }
    ...
    public static function create(
        ...
        $instance = static::rawCreateFromFormat(
            '!Y-n-j G:i:s.u',
            sprintf(
                '%s-%s-%s %s:%02s:%02s',
                $year,
    ...
    public static function createSafe(
        ...
        foreach ($fields as $field => $range) {
            if (
                $$field !== null &&
                (!is_int($$field) || $$field < $range[0] || $$field > $range[1])
            ) {
        ...
        foreach (array_reverse($fields) as $field => $range) {
            if (
                $$field !== null &&
                (!is_int($$field) || $$field !== $instance->$field)
            ) {
                if (static::isStrictModeEnabled()) {
                    throw new InvalidDateException($field, $$field);
                }
     ...
     * @return static|CarbonInterface
     */
    public static function createMidnightDate(
        $year = null,
        $month = null,
        $day = null,
        $tz = null
    ...
    public static function rawCreateFromFormat($format, $time, $tz = null)
    {
        ...
        if (
            ...
            if (
                $tz === null &&
                !preg_match("/{$nonEscaped}[eOPT]/", $nonIgnored)
            ) {
                $tz = $mock->getTimezone();
            }
            ...
            if (!preg_match("/{$nonEscaped}[!|]/", $format)) {
                ...
                $time =
                    ($mock instanceof self
                        ? $mock->rawFormat(static::MOCK_DATETIME_FORMAT)
                        : $mock->format(static::MOCK_DATETIME_FORMAT)) .
                    ' ' .
                    $time;
            }
        ...
        if (static::isStrictModeEnabled()) {
            throw new InvalidArgumentException(
                implode(PHP_EOL, $lastErrors['errors'])
            );
        }
    ...
    public static function createFromIsoFormat(
        ...
        $format = preg_replace_callback(
            ...
            function ($match) use ($locale, $translator) {
                ...
                if ($formats === null) {
                    ...
                    $formats = [
                        'LT' => static::getTranslationMessageWith(
                            $translator,
                            'formats.LT',
                            $locale,
                            'h:mm A'
                        ),
                        'LTS' => static::getTranslationMessageWith(
                            $translator,
                            'formats.LTS',
                            $locale,
                            'h:mm:ss A'
                        ),
                        'L' => static::getTranslationMessageWith(
                            $translator,
                            'formats.L',
                            $locale,
                            'MM/DD/YYYY'
                        ),
                        'LL' => static::getTranslationMessageWith(
                            $translator,
                            'formats.LL',
                            $locale,
                            'MMMM D, YYYY'
                        ),
                        'LLL' => static::getTranslationMessageWith(
                            $translator,
                            'formats.LLL',
                            $locale,
                            'MMMM D, YYYY h:mm A'
                        ),
                        'LLLL' => static::getTranslationMessageWith(
                            $translator,
                            'formats.LLLL',
                            $locale,
        ...
        $format = preg_replace_callback(
            '/(?<!\\\\)(\\\\{2})*(' .
                CarbonInterface::ISO_FORMAT_REGEXP .
                '|[A-Za-z])/',
            function ($match) {
                [$code] = $match;
     ...
     * @return static|CarbonInterface|false
     */
    public static function createFromLocaleFormat(
        $format,
        $locale,
        $time,
        $tz = null
    ) {
        return static::rawCreateFromFormat(
            $format,
            static::translateTimeString($time, $locale, 'en'),
            $tz
        );
     ...
     * @return static|CarbonInterface|false
     */
    public static function createFromLocaleIsoFormat(
        $format,
        $locale,
        $time,
        $tz = null
    ) {
        $time = static::translateTimeString(
            $time,
            $locale,
            'en',
