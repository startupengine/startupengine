...
class Translator extends Translation\Translator
{
    ...
    public function __construct(
        $locale,
        Translation\Formatter\MessageFormatterInterface $formatter = null,
        $cacheDir = null,
        $debug = false
    ) {
    ...
    public function removeDirectory(string $directory)
    {
        ...
        return $this->setDirectories(
            array_filter($this->getDirectories(), function ($item) use (
                $search
            ) {
    ...
    public function setLocale($locale)
    {
        ...
        if (
            ...
            $this->loadMessagesFromFile(
                ($macroLocale = preg_replace('/^([^_]+).*$/', '$1', $locale))
            )
        ) {
