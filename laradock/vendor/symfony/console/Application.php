...
class Application
{
    ...
    public function run(
        ...
        if (($phpHandler = set_exception_handler($renderException))) {
            ...
            if (
                ...
                ($debugHandler = $phpHandler[0]->setExceptionHandler(
                    $renderException
                ))
            ) {
    ...
    public function doRun(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$name) {
            ...
            $definition->setArguments(
                array_merge($definition->getArguments(), [
        ...
        try {
            ...
            if (
                ...
                1 !== \count(($alternatives = $e->getAlternatives())) ||
                !$input->isInteractive()
            ) {
    ...
    public function find($name)
    {
        ...
        $aliases = [];
        $allCommands = $this->commandLoader
            ? array_merge(
        ...
        if (
            empty($commands) ||
            \count(preg_grep('{^' . $expr . '$}i', $commands)) < 1
        ) {
        ...
        if (\count($commands) > 1) {
            $commandList = $this->commandLoader
                ? array_merge(
                    array_flip($this->commandLoader->getNames()),
                    $this->commands
                )
                : $this->commands;
            $commands = array_unique(
                array_filter($commands, function ($nameOrAlias) use (
                    $commandList,
                    $commands,
                    &$aliases
                ) {
                    $commandName =
                        $commandList[$nameOrAlias] instanceof Command
                            ? $commandList[$nameOrAlias]->getName()
                            : $nameOrAlias;
                    $aliases[$nameOrAlias] = $commandName;

                    return $commandName === $nameOrAlias ||
                        !\in_array($commandName, $commands);
                })
            );
        }
        ...
        if (\count($commands) > 1 && !$exact) {
            ...
            $abbrevs = array_map(function ($cmd) use (
                $commandList,
                $usableWidth,
                $maxLen
            ) {
                ...
                $abbrev =
                    str_pad($cmd, $maxLen, ' ') .
                    ' ' .
                    $commandList[$cmd]->getDescription();

                return Helper::strlen($abbrev) > $usableWidth
                    ? Helper::substr($abbrev, 0, $usableWidth - 3) . '...'
                    : $abbrev;
            },
        ...
        if (null !== $this->runningCommand) {
            $output->writeln(
                sprintf(
                    '<info>%s</info>',
                    sprintf(
                        $this->runningCommand->getSynopsis(),
                        $this->getName()
                    )
                ),
    ...
    protected function doRenderException(\Exception $e, OutputInterface $output)
    {
        do {
            ...
            if (
                ...
                $class =
                    'c' === $class[0] &&
                    0 === strpos($class, "class@anonymous\0")
            ...
            foreach (
                ...
                foreach (
                    $this->splitStringByWidth($line, $width - 4)
                    as $line
                ) {
            ...
            if (
                ...
                $messages[] = sprintf(
                    '<error>%s%s</error>',
                    $title,
                    str_repeat(' ', max(0, $len - Helper::strlen($title)))
                );
            }
    ...
    private function findAlternatives($name, $collection)
    {
        ...
        foreach (explode(':', $name) as $i => $subname) {
            foreach ($collectionParts as $collectionName => $parts) {
                ...
                if (
                    $lev <= \strlen($subname) / 3 ||
                    ('' !== $subname && false !== strpos($parts[$i], $subname))
                ) {
        ...
        $alternatives = array_filter($alternatives, function ($lev) use (
            $threshold
        ) {
