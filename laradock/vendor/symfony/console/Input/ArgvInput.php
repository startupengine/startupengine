...
class ArgvInput extends Input
{
    ...
    private function parseShortOption($token)
    {
        ...
        if (\strlen($name) > 1) {
            if (
                $this->definition->hasShortcut($name[0]) &&
    ...
    private function parseArgument($token)
    {
        ...
        if ($this->definition->hasArgument($c)) {
            ...
            $this->definition->hasArgument($c - 1) &&
            ...
            throw new RuntimeException(
                sprintf('No arguments expected, got "%s".', $token)
            );
        }
    }
    ...
    private function addShortOption($shortcut, $value)
    {
        ...
        $this->addLongOption(
            $this->definition->getOptionForShortcut($shortcut)->getName(),
            $value
        );
    }
    ...
    private function addLongOption($name, $value)
    {
        ...
        if (null !== $value && !$option->acceptValue()) {
            throw new RuntimeException(
                sprintf('The "--%s" option does not accept a value.', $name)
            );
        }
    ...
    public function getFirstArgument()
    {
        ...
        foreach ($this->tokens as $i => $token) {
            if ($token && '-' === $token[0]) {
                ...
                $name =
                    '-' === $token[1] ? substr($token, 2) : substr($token, -1);
                if (
                    !isset($this->options[$name]) &&
                    !$this->definition->hasShortcut($name)
                ) {
                    // noop
                } elseif (
                    (isset($this->options[$name]) ||
                        isset(
                            $this->options[
                                ($name = $this->definition->shortcutToName(
                                    $name
                                ))
                            ]
                        )) &&
    ...
    public function hasParameterOption($values, $onlyParams = false)
    {
        ...
        foreach ($this->tokens as $token) {
            ...
            foreach ($values as $value) {
                ...
                if (
                    $token === $value ||
                    ('' !== $leading && 0 === strpos($token, $leading))
                ) {
                    return true;
                }
            }
