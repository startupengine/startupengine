...
class ErrorListenerTest extends TestCase
{
    public function testOnConsoleError()
    {
        ...
        $logger
            ...
            ->with(
                ...
                [
                    'exception' => $error,
                    'command' => 'test:run --foo=baz buzz',
                    'message' => 'An error occurred'
                ]
            );

        $listener = new ErrorListener($logger);
        $listener->onConsoleError(
            new ConsoleErrorEvent(
                new ArgvInput(['console.php', 'test:run', '--foo=baz', 'buzz']),
                $this->getOutput(),
                $error,
    ...
    public function testOnConsoleTerminateForNonZeroExitCodeWritesToLog()
    {
        ...
        $listener->onConsoleTerminate(
            $this->getConsoleTerminateEvent(
                new ArgvInput(['console.php', 'test:run']),
                255
            )
        );
    }
    ...
    public function testAllKindsOfInputCanBeLogged()
    {
        ...
        $listener->onConsoleTerminate(
            $this->getConsoleTerminateEvent(
                new ArgvInput(['console.php', 'test:run', '--foo=bar']),
                255
            )
        );
        $listener->onConsoleTerminate(
            $this->getConsoleTerminateEvent(
                new ArrayInput(['name' => 'test:run', '--foo' => 'bar']),
                255
            )
        );
        $listener->onConsoleTerminate(
            $this->getConsoleTerminateEvent(
                new StringInput('test:run --foo=bar'),
                255
            )
        );
    }
