...
class ProcessHelperTest extends TestCase
{
    ...
    {
        if (\is_string($cmd)) {
            $cmd = \method_exists(Process::class, 'fromShellCommandline')
...
EOT;
        ...
        $successOutputProcessDebug = str_replace(
            "'php' '-r' 'echo 42;'",
            $args,
            $successOutputProcessDebug
        );
        $fromShellCommandline = \method_exists(
            Process::class,
            'fromShellCommandline'
        )
            ? [Process::class, 'fromShellCommandline']
            : function ($cmd) {
        ...
        return [
            ...
            [
                $successOutputPhp,
                [$fromShellCommandline('php -r ' . $PHP), 'PHP' => 'echo 42;'],
                StreamOutput::VERBOSITY_DEBUG,
                null
            ]
        ];
