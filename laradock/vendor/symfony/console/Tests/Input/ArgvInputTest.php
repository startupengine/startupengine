...
class ArgvInputTest extends TestCase
{
    ...
    public function testParseArrayArgument()
    {
        ...
        $input->bind(
            new InputDefinition([
                new InputArgument('name', InputArgument::IS_ARRAY)
            ])
        );
    ...
    public function testParseArrayOption()
    {
        ...
        $this->assertSame(
            ['name' => ['foo', 'bar', null], 'anotherOption' => true],
            $input->getOptions(),
    ...
    public function testGetFirstArgument()
    {
        ...
        $input->bind(
            new InputDefinition([
                new InputOption('bar', 'b', InputOption::VALUE_NONE),
                new InputOption('foo', 'f', InputOption::VALUE_OPTIONAL),
                new InputArgument('arg')
            ])
        );
    ...
    public function testHasParameterOptionOnlyOptions()
    {
        $input = new ArgvInput(['cli.php', '-f', 'foo']);
        $this->assertTrue(
            $input->hasParameterOption('-f', true),
            '->hasParameterOption() returns true if the given short option is in the raw input'
        );

        $input = new ArgvInput(['cli.php', '--foo', '--', 'foo']);
        $this->assertTrue(
            $input->hasParameterOption('--foo', true),
            '->hasParameterOption() returns true if the given long option is in the raw input'
        );

        $input = new ArgvInput(['cli.php', '--foo=bar', 'foo']);
        $this->assertTrue(
            $input->hasParameterOption('--foo', true),
            '->hasParameterOption() returns true if the given long option with provided value is in the raw input'
        );

        $input = new ArgvInput(['cli.php', '--', '--foo']);
        $this->assertFalse(
            $input->hasParameterOption('--foo', true),
            '->hasParameterOption() returns false if the given option is in the raw input but after an end of options signal'
        );
    }
     ...
     */
    public function testGetParameterOptionEqualSign(
        $argv,
        $key,
        $default,
        $onlyParams,
        ...
        $this->assertEquals(
            $expected,
            $input->getParameterOption($key, $default, $onlyParams),
    ...
    public function provideGetParameterOptionValues()
    {
        return [
            ...
            [
                ['app/console', 'foo:bar', '--env', 'val', '--dummy'],
                '--env',
                'default',
                false,
                'val'
    ...
    public function testParseOptionWithValueOptionalGivenEmptyAndRequiredArgument()
    {
        ...
        $this->assertEquals(
            ['foo' => null],
            $input->getOptions(),
            '->parse() parses optional options with empty value as null'
        );
        $this->assertEquals(
            ['name' => 'bar'],
            $input->getArguments(),
    ...
    public function testParseOptionWithValueOptionalGivenEmptyAndOptionalArgument()
    {
        ...
        $this->assertEquals(
            ['name' => 'bar'],
            $input->getArguments(),
            '->parse() parses optional arguments'
        );
