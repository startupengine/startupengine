...
class QuestionHelper extends Helper
{
    ...
    private function doAsk(OutputInterface $output, Question $question)
    {
        ...
        if (null === $autocomplete || !$this->hasSttyAvailable()) {
            ...
            if ($question->isHidden()) {
                try {
                    $ret = trim(
                        $this->getHiddenResponse($output, $inputStream)
                    );
            ...
            $ret = trim(
                $this->autocomplete(
                    ...
                    $question,
                    $inputStream,
                    \is_array($autocomplete)
                        ? $autocomplete
                        : iterator_to_array($autocomplete, false)
                )
            );
        }
    ...
    protected function writePrompt(OutputInterface $output, Question $question)
    {
        ...
        if ($question instanceof ChoiceQuestion) {
            $maxWidth = max(
                array_map(
                    [$this, 'strlen'],
                    array_keys($question->getChoices())
                )
            );
    ...
    protected function writeError(OutputInterface $output, \Exception $error)
    {
        if (
            ...
            $this->getHelperSet()->has('formatter')
        ) {
            $message = $this->getHelperSet()
                ->get('formatter')
    ...
    private function autocomplete(
        ...
        while (!feof($inputStream)) {
            ...
            if (
                false === $c ||
                ('' === $ret && '' === $c && null === $question->getDefault())
            ) {
                throw new RuntimeException('Aborted.');
            } elseif ("\177" === $c) {
                // Backspace Character
     ...
     * @throws RuntimeException In case the fallback is deactivated and the response cannot be hidden
     */
    private function getHiddenResponse(
        OutputInterface $output,
        $inputStream
    ): string {
