...
class ProcessHelper extends Helper
{
    ...
    public function run(
        ...
        if (!\is_array($cmd)) {
            @trigger_error(
                sprintf(
                    'Passing a command as a string to "%s()" is deprecated since Symfony 4.2, pass it the command as an array of arguments instead.',
                    __METHOD__
                ),
                E_USER_DEPRECATED
            );
        ...
        if (!$process->isSuccessful() && null !== $error) {
            $output->writeln(
                sprintf('<error>%s</error>', $this->escapeString($error))
            );
        }
    ...
    public function wrapCallback(
        ...
        return function ($type, $buffer) use (
            $output,
            $process,
            $callback,
            $formatter
