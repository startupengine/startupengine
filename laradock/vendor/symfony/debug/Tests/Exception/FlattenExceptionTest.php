...
class FlattenExceptionTest extends TestCase
{
    public function testStatusCode()
    {
        ...
        $flattened = FlattenException::createFromThrowable(
        ...
        $flattened = FlattenException::create(
            new UnauthorizedHttpException('Basic realm="My Realm"')
        );
    ...
    public function testHeadersForHttpException()
    {
        ...
        $flattened = FlattenException::create(
            new ServiceUnavailableHttpException('Fri, 31 Dec 1999 23:59:59 GMT')
        );
        $this->assertEquals(
        ...
        $flattened = FlattenException::create(
            new ServiceUnavailableHttpException(120)
        );
        $this->assertEquals(['Retry-After' => 120], $flattened->getHeaders());

        $flattened = FlattenException::create(
            new TooManyRequestsHttpException('Fri, 31 Dec 1999 23:59:59 GMT')
        );
        $this->assertEquals(
            ['Retry-After' => 'Fri, 31 Dec 1999 23:59:59 GMT'],
            $flattened->getHeaders()
        ...
        $flattened = FlattenException::create(
            new TooManyRequestsHttpException(120)
        );
        $this->assertEquals(['Retry-After' => 120], $flattened->getHeaders());
    }
    ...
    {
        $exception = new FatalThrowableError(
            new \DivisionByZeroError('Ouch', 42)
        );
        ...
        $this->assertSame(
            'Ouch',
            $flattened->getMessage(),
            'The message is copied from the original error.'
        );
        $this->assertSame(
            42,
            $flattened->getCode(),
            'The code is copied from the original error.'
        );
        $this->assertSame(
            'DivisionByZeroError',
            $flattened->getClass(),
            'The class is set to the class of the original error'
        );
    }
    ...
    public function testTooBigArray()
    {
        ...
        $this->assertSame($trace[1]['args'][0], [
            'array',
    ...
    public function testAnonymousClass()
    {
        ...
        $flattened = FlattenException::create(
            new \Exception(
                sprintf(
                    'Class "%s" blah.',
                    \get_class(new class extends \RuntimeException {})
                )
            )
        );

        $this->assertSame(
            'Class "RuntimeException@anonymous" blah.',
            $flattened->getMessage()
        );
    }
