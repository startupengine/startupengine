...
class ErrorHandlerTest extends TestCase
{
    public function testRegister()
    {
        ...
        try {
            ...
            $this->assertSame(
                $handler,
                ErrorHandler::register($newHandler, false)
            );
            ...
            try {
                $this->assertSame(
                    $newHandler,
                    ErrorHandler::register($newHandler, true)
                );
                $h = set_error_handler('var_dump');
                restore_error_handler();
    ...
    public function testNotice()
    {
        ...
        try {
            ...
            $this->assertRegExp(
                '/^Notice: Undefined variable: (foo|bar)/',
    ...
    public function testConstruct()
    {
        try {
            ...
            $this->assertEquals(
                3 | E_RECOVERABLE_ERROR | E_USER_ERROR,
                $handler->throwAt(0)
            );
        } finally {
            restore_error_handler();
    ...
    public function testHandleError()
    {
        try {
            ...
            $this->assertFalse(
                $handler->handleError(0, 'foo', 'foo.php', 12, [])
            );
            ...
            $this->assertFalse(
                $handler->handleError(E_DEPRECATED, 'foo', 'foo.php', 12, [])
            );
            ...
            $logArgCheck = function ($level, $message, $context) use (&$line) {
                $this->assertEquals(
                    'Notice: Undefined variable: undefVar',
                    $message
                );
                $this->assertArrayHasKey('exception', $context);
                $exception = $context['exception'];
    ...
    public function testHandleDeprecation()
    {
        $logArgCheck = function ($level, $message, $context) {
            ...
            $this->assertInstanceOf(\ErrorException::class, $exception);
            $this->assertSame(
                'User Deprecated: Foo deprecation',
                $exception->getMessage()
            );
        };
    ...
    public function testBootstrappingLogger()
    {
        ...
        $bootLogger->log(LogLevel::WARNING, 'Foo message', [
            'exception' => $exception
        ]);

        $mockLogger = $this->getMockBuilder(
            'Psr\Log\LoggerInterface'
        )->getMock();
        $mockLogger
            ->expects($this->once())
            ->method('log')
            ->with(LogLevel::WARNING, 'Foo message', [
                'exception' => $exception
            ]);

        $handler->setLoggers([
            E_DEPRECATED => [$mockLogger, LogLevel::WARNING]
        ]);
    }
    ...
    public function testSettingLoggerWhenExceptionIsBuffered()
    {
        ...
        $mockLogger
            ->expects($this->once())
            ->method('log')
            ->with(LogLevel::CRITICAL, 'Uncaught Exception: Foo message', [
                'exception' => $exception
            ]);
    ...
    public function testHandleErrorException()
    {
        ...
        $this->assertStringStartsWith(
            "Attempted to load class \"Foo\" from the global namespace.\nDid you forget a \"use\" statement",
            $args[0]->getMessage()
        );
    }
     ...
     */
    public function testErrorHandlerWhenLogging(
        $previousHandlerWasDefined,
        $loggerSetsAnotherHandler,
        $nextHandlerIsDefined
    ) {
        try {
            ...
            $logger = $loggerSetsAnotherHandler
