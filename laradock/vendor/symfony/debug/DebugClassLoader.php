...
class DebugClassLoader
{
    ...
    public function __construct(callable $classLoader)
    {
        ...
        $this->isFinder =
            \is_array($classLoader) &&
            method_exists($classLoader[0], 'findFile');

        if (!isset(self::$caseCheck)) {
            $file = file_exists(__FILE__)
                ? __FILE__
    ...
    public function loadClass($class)
    {
        ...
        try {
            if ($this->isFinder && !isset($this->loaded[$class])) {
                ...
                if (
                    !($file = $this->classLoader[0]->findFile($class) ?: false)
                ) {
                    // no-op
                } elseif (
    ...
    private function checkClass($class, $file = null)
    {
        ...
        if (
            self::$caseCheck &&
            ($message = $this->checkCase($refl, $file, $class))
        ) {
            throw new \RuntimeException(
                sprintf(
    ...
    public function checkAnnotations(\ReflectionClass $refl, $class)
    {
        ...
        if (false !== ($doc = $refl->getDocComment())) {
            foreach (['final', 'deprecated', 'internal'] as $annotation) {
                if (
                    ...
                    self::${$annotation}[$class] = isset($notice[1])
                        ? preg_replace(
                            '#\.?\r?\n( \*)? *(?= |\r?\n|$)#',
                            '',
                            $notice[1]
        ...
        foreach ($parentAndOwnInterfaces + \class_uses($class, false) as $use) {
            ...
            if (
                isset(self::$deprecated[$use]) &&
                \strncmp($ns, \str_replace('_', '\\', $use), $len)
            ) {
                $type = class_exists($class, false)
                    ? 'class'
                    : (interface_exists($class, false)
                        ? 'interface'
                        : 'trait');
                $verb =
                    class_exists($use, false) || interface_exists($class, false)
                        ? 'extends'
                        : (interface_exists($use, false)
                            ? 'implements'
                            : 'uses');
        ...
        foreach ($parentAndOwnInterfaces as $use) {
            foreach (
                ['finalMethods', 'internalMethods', 'annotatedParameters']
                as $property
            ) {
                if (isset(self::${$property}[$use])) {
                    self::${$property}[$class] = self::${$property}[$class]
                        ? self::${$property}[$use] + self::${$property}[$class]
                        : self::${$property}[$use];
                }
            }
        }
        ...
        foreach (
            ...
            if ($parent && isset(self::$finalMethods[$parent][$method->name])) {
                list($declaringClass, $message) = self::$finalMethods[$parent][
                    $method->name
                ];
                $deprecations[] = sprintf(
            ...
            if (isset(self::$internalMethods[$class][$method->name])) {
                list($declaringClass, $message) = self::$internalMethods[
                    $class
                ][$method->name];
            ...
            if (isset(self::$annotatedParameters[$class][$method->name])) {
                ...
                foreach (
                    self::$annotatedParameters[$class][$method->name]
                    as $parameterName => $deprecation
                ) {
                    if (
                        !isset($definedParameters[$parameterName]) &&
                        !(
                            $doc &&
                            preg_match(
                                "/\\n\\s+\\* @param (.*?)(?<= )\\\${$parameterName}\\b/",
                                $doc
                            )
                        )
                    ) {
            ...
            if (
                ...
                false === \strpos($doc, '@param') ||
                StatelessInvocation::class === $class
            ) {
                continue;
            }
            if (
            ...
            foreach ($matches as list(, $parameterType, $parameterName)) {
                if (!isset($definedParameters[$parameterName])) {
                    ...
                    self::$annotatedParameters[$class][$method->name][
                        ...
                        'The "%%s::%s()" method will require a new "%s$%s" argument in the next major version of its parent class "%s", not defining it is deprecated.',
                        $method->name,
    ...
    public function checkCase(\ReflectionClass $refl, $file, $class)
    {
        ...
        if (
            0 === substr_compare($real, $tail, -$tailLen, $tailLen, true) &&
