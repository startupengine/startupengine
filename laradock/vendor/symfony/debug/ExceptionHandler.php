...
EOF;
    ...
    {
        $file = $this->escapeHtml(
            preg_match('#[^/\\\\]*+$#', $path, $file) ? $file[0] : $path
        );
        $fmt =
            $this->fileLinkFormat ?:
    ...
    private function formatArgs(array $args)
    {
        ...
        foreach ($args as $key => $item) {
            if ('object' === $item[0]) {
                ...
                $formattedValue =
                    '<em>' . strtolower(var_export($item[1], true)) . '</em>';
