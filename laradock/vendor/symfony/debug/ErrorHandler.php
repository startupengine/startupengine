...
class ErrorHandler
{
    ...
    public static function register(self $handler = null, $replace = true)
    {
        ...
        if (
            \is_array(
                ($prev = set_exception_handler([$handler, 'handleException']))
            ) &&
            $prev[0] instanceof self
        ) {
    ...
    public function setDefaultLogger(
        ...
        if (\is_array($levels)) {
            foreach ($levels as $type => $logLevel) {
                if (
                    empty($this->loggers[$type][0]) ||
                    $replace ||
                    $this->loggers[$type][0] === $this->bootstrappingLogger
                ) {
            ...
            foreach ($this->loggers as $type => $log) {
                if (
                    ...
                    (empty($log[0]) ||
                        $replace ||
                        $log[0] === $this->bootstrappingLogger)
                ) {
                    $log[0] = $logger;
                    $loggers[$type] = $log;
                }
    ...
    public function setLoggers(array $loggers)
    {
        ...
        foreach ($loggers as $type => $log) {
            ...
            if (
                ...
                $prev[$type][0] === $this->bootstrappingLogger
            ) {
                $flush[$type] = $type;
            }
        }
        ...
        if ($flush) {
            foreach ($this->bootstrappingLogger->cleanLogs() as $log) {
                $type =
                    $log[2]['exception'] instanceof \ErrorException
                        ? $log[2]['exception']->getSeverity()
                        : E_ERROR;
                if (!isset($flush[$type])) {
    ...
    public function throwAt($levels, $replace = false)
    {
        ...
        $this->thrownErrors =
            ($levels | E_RECOVERABLE_ERROR | E_USER_ERROR) &
            ~E_USER_DEPRECATED &
            ~E_DEPRECATED;
        if (!$replace) {
    ...
    public function handleError($type, $message, $file, $line)
    {
        ...
        if (false !== strpos($message, "class@anonymous\0")) {
            $logMessage =
                $this->levels[$type] .
                ': ' .
                (new FlattenException())->setMessage($message)->getMessage();
        } else {
        ...
        if (null !== self::$toStringException) {
            ...
            if (
                !isset(self::$silencedErrorCache[($id = $file . ':' . $line)])
            ) {
                $lightTrace =
                    $this->tracedErrors & $type
                        ? $this->cleanTrace(
                        ...
                        : [];
                $errorAsException = new SilencedErrorContext(
                    $type,
                    $file,
                    $line,
                    isset($lightTrace[1]) ? [$lightTrace[0]] : $lightTrace
                );
                ...
                for ($i = 1; isset($backtrace[$i]); ++$i) {
                    if (
                        isset(
                            ...
                            $backtrace[$i]['type'],
                            $backtrace[$i - 1]['function']
                        ) &&
                        '__toString' === $backtrace[$i]['function'] &&
                        '->' === $backtrace[$i]['type'] &&
                        !isset($backtrace[$i - 1]['class']) &&
                        ('trigger_error' === $backtrace[$i - 1]['function'] ||
                            'user_error' === $backtrace[$i - 1]['function'])
                    ) {
    ...
    public function handleException($exception, array $error = null)
    {
        ...
        if (
            $exception instanceof FatalErrorException &&
            !$exception instanceof OutOfMemoryException &&
            $error
        ) {
        ...
        if (
            ...
            if (
                ...
                $exception = new OutOfMemoryException(
                    $handler->levels[$error['type']] . ': ' . $error['message'],
                    0,
                    $error['type'],
                    $error['file'],
                    $error['line'],
    ...
    private function cleanTrace($backtrace, $type, $file, $line, $throw)
    {
        ...
        for ($i = 0; isset($backtrace[$i]); ++$i) {
            if (
                isset($backtrace[$i]['file'], $backtrace[$i]['line']) &&
