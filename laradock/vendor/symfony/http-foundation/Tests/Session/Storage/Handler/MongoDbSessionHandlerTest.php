...
class MongoDbSessionHandlerTest extends TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->storage = new MongoDbSessionHandler(
            $this->mongo,
            $this->options
        );
    }
    ...
    public function testRead()
    {
        ...
        $collection
            ...
            ->method('findOne')
            ->will(
                $this->returnCallback(function ($criteria) use ($testTimeout) {
                    $this->assertArrayHasKey(
                        $this->options['id_field'],
                        $criteria
                    );
                    $this->assertEquals(
                        $criteria[$this->options['id_field']],
                        'foo'
                    );
                    ...
                    $this->assertArrayHasKey(
                        '$gte',
                        $criteria[$this->options['expiry_field']]
                    );

                    $this->assertInstanceOf(
                        \MongoDB\BSON\UTCDateTime::class,
                        $criteria[$this->options['expiry_field']]['$gte']
                    );
                    $this->assertGreaterThanOrEqual(
                        round(
                            (string) $criteria[$this->options['expiry_field']][
                                '$gte'
                            ] / 1000
                        ),
                        $testTimeout
            ...
            ->will(
                $this->returnCallback(function (
                    ...
                    $this->assertInstanceOf(
                        ...
                        $data[$this->options['data_field']]
                    );
                    $this->assertEquals(
                        'bar',
                        $data[$this->options['data_field']]->getData()
                    );
                    $this->assertInstanceOf(
                        \MongoDB\BSON\UTCDateTime::class,
                    ...
                    $this->assertGreaterThanOrEqual(
                        $expectedExpiry,
                        round(
                            (string) $data[$this->options['expiry_field']] /
                                1000
                        )
                    );
                })
    ...
    public function testGc()
    {
        ...
        $collection
            ...
            ->method('deleteMany')
            ->will(
                $this->returnCallback(function ($criteria) {
                    $this->assertInstanceOf(
                        \MongoDB\BSON\UTCDateTime::class,
                        $criteria[$this->options['expiry_field']]['$lt']
                    );
                    $this->assertGreaterThanOrEqual(
                        time() - 1,
                        round(
