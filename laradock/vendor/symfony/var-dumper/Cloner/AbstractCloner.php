...
abstract class AbstractCloner implements ClonerInterface
{
    ...
    public function addCasters(array $casters)
    {
        foreach ($casters as $type => $callback) {
            $this->casters[strtolower($type)][] =
                \is_string($callback) && false !== strpos($callback, '::')
                    ? explode('::', $callback, 2)
                    : $callback;
        }
    }
    ...
    {
        $this->prevErrorHandler = set_error_handler(function (
            $type,
            $msg,
            $file,
            $line,
    ...
    protected function castObject(Stub $stub, $isNested)
    {
        ...
        if (
            isset($class[15]) &&
            "\0" === $class[15] &&
            0 === strpos($class, "class@anonymous\x00")
        ) {
