    ...
    public function getContext(): ?array
    {
            ...
            if (
                ...
                while (++$i < $this->limit) {
                    if (
                        ...
                        empty($trace[$i]['class']) &&
                        0 !== strpos($trace[$i]['function'], 'call_user_func')
                    ) {
                        $file = $trace[$i]['file'];
                        $line = $trace[$i]['line'];
                        ...
                        $trace[$i]['object'] instanceof Template
                    ) {
                        $template = $trace[$i]['object'];
                        $name = $template->getTemplateName();
                        $src = method_exists($template, 'getSourceContext')
                        ...
                        if (isset($info[$trace[$i - 1]['line']])) {
                            ...
                            $file = method_exists($template, 'getSourceContext')
        ...
        if (null !== $this->projectDir) {
            ...
            if (0 === strpos($file, $this->projectDir)) {
                $context['file_relative'] = ltrim(
                    substr($file, \strlen($this->projectDir)),
                    \DIRECTORY_SEPARATOR
                );
            }
        }
        ...
        if (
            ...
            ($fileLink = $this->fileLinkFormatter->format(
                $context['file'],
                $context['line']
            ))
        ) {
