...
class TestSessionListenerTest extends TestCase
{
     ...
     */
    public function testSessionWithNewSessionIdAndNewCookieDoesNotSendAnotherCookie(
        $existing,
        array $expected
    ) {
        $this->sessionHasBeenStarted();
        ...
        $this->assertSame(
            $expected,
            $response->headers->get('Set-Cookie', null, false)
        );
    }
    ...
    public function anotherCookieProvider()
    {
        return [
            ...
            'different domain' => [
                'MOCKSESSID=789; path=/; domain=example.com',
                [
                    'MOCKSESSID=789; path=/; domain=example.com',
                    'MOCKSESSID=456; path=/'
                ]
            ...
            'different path' => [
                'MOCKSESSID=789; path=/foo',
                ['MOCKSESSID=789; path=/foo', 'MOCKSESSID=456; path=/']
            ]
        ];
    }
    ...
    private function getSession()
    {
        ...
        $mock
            ->expects($this->any())
            ->method('getName')
            ->will($this->returnValue('MOCKSESSID'));
