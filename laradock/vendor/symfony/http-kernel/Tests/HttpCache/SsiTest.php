...
class SsiTest extends TestCase
{
    ...
    public function testAddSurrogateSsiCapability()
    {
        ...
        $ssi->addSurrogateCapability($request);
        $this->assertEquals(
            'symfony="SSI/1.0", symfony="SSI/1.0"',
            $request->headers->get('Surrogate-Capability')
        );
    }
    ...
    public function testRenderIncludeTag()
    {
        ...
        $this->assertEquals(
            '<!--#include virtual="/" -->',
            $ssi->renderIncludeTag('/', '/alt', true)
        );
        $this->assertEquals(
            '<!--#include virtual="/" -->',
            $ssi->renderIncludeTag('/', '/alt', false)
        );
        $this->assertEquals(
            '<!--#include virtual="/" -->',
    ...
    public function testProcess()
    {
        ...
        $this->assertEquals(
            "foo <?php echo \$this->surrogate->handle(\$this, 'foo\\'', '', false) ?>" .
                "\n",
            $response->getContent()
        );
    }
