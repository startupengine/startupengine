...
class ResponseCacheStrategyTest extends TestCase
{
    ...
    public function testMasterResponseNotCacheableWhenEmbeddedResponseRequiresValidation()
    {
        ...
        $this->assertTrue(
            $masterResponse->headers->hasCacheControlDirective('no-cache')
        );
        $this->assertTrue(
            $masterResponse->headers->hasCacheControlDirective(
                'must-revalidate'
            )
        );
    ...
    public function testEmbeddingPrivateResponseMakesMainResponsePrivate()
    {
        ...
        $this->assertTrue(
            $masterResponse->headers->hasCacheControlDirective('private')
        );
        $this->assertFalse(
            $masterResponse->headers->hasCacheControlDirective('public')
        );
    }
    ...
    public function testResponseIsExiprableWhenEmbeddedResponseCombinesExpiryAndValidation()
    {
        ...
        $this->assertSame(
            '60',
            $masterResponse->headers->getCacheControlDirective('s-maxage')
        );
    }
     ...
     */
    public function testCacheControlMerging(
        array $expects,
        array $master,
        array $surrogates
    ) {
        ...
        foreach ($expects as $key => $value) {
            if ('expires' === $key) {
                ...
                $this->assertTrue(
                    ...
                    sprintf('Cache-Control header must have "%s" flag', $key)
                );
            } elseif (false === $value) {
                $this->assertFalse(
                    $response->headers->hasCacheControlDirective($key),
                    sprintf(
                        'Cache-Control header must NOT have "%s" flag',
                        $key
                    )
                );
            } else {
                $this->assertSame(
                    $value,
                    $response->headers->getCacheControlDirective($key),
                    sprintf(
                        'Cache-Control flag "%s" should be "%s"',
                        $key,
                        $value
                    )
                );
