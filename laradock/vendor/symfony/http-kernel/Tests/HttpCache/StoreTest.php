...
class StoreTest extends TestCase
{
    ...
    public function testSetsTheXContentDigestResponseHeaderBeforeStoring()
    {
        ...
        $this->assertEquals(
            'en9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08',
            $res['x-content-digest'][0]
        );
    }
    ...
    public function testDoesNotFindAnEntryWithLookupWhenTheBodyDoesNotExist()
    {
        ...
        $path = $this->getStorePath(
            $this->response->headers->get('X-Content-Digest')
        );
        @unlink($path);
        ...
        $this->assertEquals(
            ...
            array_merge(
                [
                    ...
                    'x-body-file' => [
                        $this->getStorePath(
                            $response->headers->get('X-Content-Digest')
                        )
                    ]
                ],
    ...
    public function testRestoresResponseContentFromEntityStoreWithLookup()
    {
        ...
        $response = $this->store->lookup($this->request);
        $this->assertEquals(
            $this->getStorePath('en' . hash('sha256', 'test')),
            $response->getContent()
        );
    }
    ...
    public function testStoresMultipleResponsesForEachVaryCombination()
    {
        ...
        $req3 = Request::create(
            ...
            [],
            [],
            ['HTTP_FOO' => 'Baz', 'HTTP_BAR' => 'Boom']
        );
        $res3 = new Response('test 3', 200, ['Vary' => 'Foo Bar']);
        $this->store->write($req3, $res3);

        $this->assertEquals(
            $this->getStorePath('en' . hash('sha256', 'test 3')),
            $this->store->lookup($req3)->getContent()
        );
        $this->assertEquals(
            $this->getStorePath('en' . hash('sha256', 'test 2')),
            $this->store->lookup($req2)->getContent()
        );
        $this->assertEquals(
            $this->getStorePath('en' . hash('sha256', 'test 1')),
            $this->store->lookup($req1)->getContent()
        );
    ...
    protected function storeSimpleEntry($path = null, $headers = [])
    {
        ...
        $this->response = new Response('test', 200, [
