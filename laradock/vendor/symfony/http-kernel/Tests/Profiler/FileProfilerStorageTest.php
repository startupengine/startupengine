...
class FileProfilerStorageTest extends TestCase
{
    ...
    public function testStore()
    {
        ...
        $this->assertCount(
            10,
            $this->storage->find('127.0.0.1', 'http://foo.bar', 20, 'GET'),
    ...
    public function testChildren()
    {
        ...
        $this->assertEquals(
            $parentProfile->getToken(),
            $childProfile->getParentToken()
        );
    ...
    public function testStoreSpecialCharsInUrl()
    {
        ...
        $this->storage->write($profile);
        $this->assertNotFalse(
            $this->storage->read('simple_quote'),
            '->write() accepts single quotes in URL'
        );
        ...
        $this->storage->write($profile);
        $this->assertNotFalse(
            $this->storage->read('double_quote'),
            '->write() accepts double quotes in URL'
        );
        ...
        $this->storage->write($profile);
        $this->assertNotFalse(
            $this->storage->read('backslash'),
            '->write() accepts backslash in URL'
        );
        ...
        $this->storage->write($profile);
        $this->assertNotFalse(
            $this->storage->read('comma'),
            '->write() accepts comma in URL'
        );
    }
    ...
    public function testStoreDuplicateToken()
    {
        ...
        $this->assertTrue(
            $this->storage->write($profile),
            '->write() returns true when the token is already present in the storage'
        );
        $this->assertEquals(
            'http://example.net/',
            $this->storage->read('token')->getUrl(),
            '->write() overwrites the current profile data'
        );

        $this->assertCount(
            1,
            $this->storage->find('', '', 1000, ''),
            '->find() does not return the same profile twice'
        );
    }
    ...
    public function testRetrieveByIp()
    {
        ...
        $this->assertCount(
            1,
            $this->storage->find('127.0.0.1', '', 10, 'GET'),
            '->find() retrieve a record by IP'
        );
        $this->assertCount(
            0,
            $this->storage->find('127.0.%.1', '', 10, 'GET'),
            '->find() does not interpret a "%" as a wildcard in the IP'
        );
        $this->assertCount(
            0,
            $this->storage->find('127.0._.1', '', 10, 'GET'),
            '->find() does not interpret a "_" as a wildcard in the IP'
        );
    }
    ...
    public function testRetrieveByStatusCode()
    {
        ...
        $this->assertCount(
            1,
            $this->storage->find(null, null, 10, null, null, null, '200'),
            '->find() retrieve a record by Status code 200'
        );
        $this->assertCount(
            1,
            $this->storage->find(null, null, 10, null, null, null, '404'),
            '->find() retrieve a record by Status code 404'
        );
    }
    ...
    public function testRetrieveByUrl()
    {
        ...
        $this->assertCount(
            1,
            $this->storage->find('127.0.0.1', 'http://foo.bar/\'', 10, 'GET'),
            '->find() accepts single quotes in URLs'
        );
        $this->assertCount(
            1,
            $this->storage->find('127.0.0.1', 'http://foo.bar/"', 10, 'GET'),
            '->find() accepts double quotes in URLs'
        );
        $this->assertCount(
            1,
            $this->storage->find('127.0.0.1', 'http://foo\\bar/', 10, 'GET'),
            '->find() accepts backslash in URLs'
        );
        $this->assertCount(
            1,
            $this->storage->find('127.0.0.1', 'http://foo.bar/;', 10, 'GET'),
            '->find() accepts semicolon in URLs'
        );
        $this->assertCount(
            1,
            $this->storage->find('127.0.0.1', 'http://foo.bar/%', 10, 'GET'),
            '->find() does not interpret a "%" as a wildcard in the URL'
        );
        $this->assertCount(
            1,
            $this->storage->find('127.0.0.1', 'http://foo.bar/_', 10, 'GET'),
            '->find() does not interpret a "_" as a wildcard in the URL'
        );
    }
    ...
    public function testStoreTime()
    {
        ...
        $this->assertCount(
            3,
            $records,
            '->find() returns all previously added records'
        );
        $this->assertEquals(
            $records[0]['token'],
            'time_2',
            '->find() returns records ordered by time in descendant order'
        );
        $this->assertEquals(
            $records[1]['token'],
            'time_1',
        ...
        $this->assertCount(
            2,
            $records,
            '->find() should return only first two of the previously added records'
        );
    }
    ...
    public function testPurge()
    {
        ...
        $this->assertEmpty(
            ...
            '->purge() removes all data stored by profiler'
        );
        $this->assertCount(
            0,
            $this->storage->find('127.0.0.1', '', 10, 'GET'),
            '->purge() removes all items from index'
        );
    }
    ...
    public function testDuplicates()
    {
        ...
        $this->assertCount(
            3,
            $this->storage->find('127.0.0.1', 'http://example.net/', 3, 'GET'),
            '->find() method returns incorrect number of entries'
        );
    }
