...
class FragmentRendererPassTest extends TestCase
{
    ...
    public function testContentRendererWithoutInterface()
    {
        ...
        $this->assertEquals(
            [['addRendererService', ['foo', 'my_content_renderer']]],
            $fragmentHandlerDefinition->getMethodCalls()
        );
    }
    ...
    {
        $builder = new ContainerBuilder();
        $fragmentHandlerDefinition = $builder
        ...
        $serviceLocatorDefinition = $builder->getDefinition(
            (string) $fragmentHandlerDefinition->getArgument(0)
        );
        $this->assertSame(
            ServiceLocator::class,
            $serviceLocatorDefinition->getClass()
        );
        $this->assertEquals(
            [
                'foo' => new ServiceClosureArgument(
                    new Reference('my_content_renderer')
                )
            ],
            $serviceLocatorDefinition->getArgument(0)
        );
    }
}
