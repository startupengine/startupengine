...
class ProcessTest extends TestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$phpBin =
            getenv('SYMFONY_PROCESS_PHP_TEST_BINARY') ?:
            ('phpdbg' === \PHP_SAPI
                ? 'php'
                : $phpBin->find());
    ...
    public function testStopWithTimeoutIsActuallyWorking()
    {
        $p = $this->getProcess([
            self::$phpBin,
            __DIR__ . '/NonStopableProcess.php',
            30
        ]);
        $p->start();
        ...
        while (
            $p->isRunning() &&
            false === strpos($p->getOutput(), 'received')
        ) {
            usleep(1000);
        }
        ...
        if (!$p->isRunning()) {
            throw new \LogicException(
                'Process is not running: ' . $p->getErrorOutput()
            );
        }
    ...
    public function testWaitUntilSpecificOutput()
    {
        if ('\\' === \DIRECTORY_SEPARATOR) {
            $this->markTestIncomplete(
                'This test is too transient on Windows, help wanted to improve it'
            );
        }

        $p = $this->getProcess([
            self::$phpBin,
            __DIR__ . '/KillableProcessWithOutput.php'
        ]);
        $p->start();
        ...
        $completeOutput = '';
        $result = $p->waitUntil(function ($type, $output) use (
            &$completeOutput
        ) {
    ...
    {
        return [[null, null], ['24.5', 24.5], ['input data', 'input data']];
    }
    ...
    public function testChainedCommandsOutput($expected, $operator, $input)
    {
        $process = $this->getProcess(
            sprintf('echo %s %s echo %s', $input, $operator, $input)
        );
        $process->run();
    ...
    public function testGetErrorOutput()
    {
        ...
        $this->assertEquals(
            3,
            preg_match_all('/ERROR/', $p->getErrorOutput(), $matches)
        );
    }
     ...
     */
    public function testIncrementalOutput(
        $getOutput,
    ...
    public function testGetOutput()
    {
        ...
        $this->assertEquals(
            3,
            preg_match_all('/foo/', $p->getOutput(), $matches)
        );
    }
    ...
    public function testTTYCommand()
    {
        ...
        $process = $this->getProcess(
            'echo "foo" >> /dev/null && ' .
                $this->getProcessForCode('usleep(100000);')->getCommandLine()
        );
        $process->setTty(true);
        $process->start();
    ...
    {
        $process = $this->getProcessForCode(
     ...
     */
    public function testIncrementalOutputDoesNotRequireAnotherCall(
        $stream,
        $method
    ) {
        $process = $this->getProcessForCode(
            '$n = 0; while ($n < 3) { file_put_contents(\'' .
                $stream .
                '\', $n, 1); $n++; usleep(1000); }',
            null,
            null,
    ...
    public function testSimpleInputStream()
    {
        ...
        $process = $this->getProcessForCode(
            'echo \'ping\'; echo fread(STDIN, 4); echo fread(STDIN, 4);'
        );
    ...
    public function testIteratorOutput()
    {
        ...
        $expectedOutput = [[$process::OUT, '123']];
    ...
    public function testNonBlockingNorClearingIteratorOutput()
    {
        ...
        foreach (
            $process->getIterator(
                $process::ITER_NON_BLOCKING | $process::ITER_KEEP_OUTPUT
            )
            as $type => $data
        ) {
            $output[] = [$type, $data];
        ...
        $expectedOutput = [[$process::OUT, ''], [$process::OUT, '123']];
        $this->assertSame($expectedOutput, $output);
    }

    public function testChainedProcesses()
    {
        $p1 = $this->getProcessForCode(
    ...
    {
        $process = $this->getProcessForCode('echo serialize($_SERVER);', null, [
            'BAR' => 'BAZ',
            'EMPTY' => ''
        ]);
...
EOTXT;
        $this->assertSame(
            $expected,
            str_replace('Standard input code', '-', $p->getOutput())
        );
    }
    ...
    private function getProcessForCode(
        string $code,
        string $cwd = null,
        array $env = null,
        $input = null,
