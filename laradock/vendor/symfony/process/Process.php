...
class Process implements \IteratorAggregate
{
    ...
    public function __construct(
        ...
        if (!\is_array($command)) {
            @trigger_error(
                sprintf(
                    'Passing a command as string when creating a "%s" instance is deprecated since Symfony 4.2, pass it as an array of its arguments instead, or use the "Process::fromShellCommandline()" constructor if you need features provided by the shell.',
                    __CLASS__
                ),
                E_USER_DEPRECATED
            );
    ...
    public function start(callable $callback = null, array $env = [])
    {
        ...
        if (\is_array(($commandline = $this->commandline))) {
            $commandline = implode(
                ' ',
                array_map([$this, 'escapeArgument'], $commandline)
            );
        ...
        if (isset($descriptors[3])) {
            $this->fallbackStatus['pid'] = (int) fgets(
                $this->processPipes->pipes[3]
            );
        }
    ...
    public function wait(callable $callback = null)
    {
        ...
        if (
            $this->processInformation['signaled'] &&
            $this->processInformation['termsig'] !== $this->latestSignal
        ) {
        ...
        while (true) {
            ...
            $running =
                '\\' === \DIRECTORY_SEPARATOR
                    ? $this->isRunning()
                    : $this->processPipes->areOpen();
            $output = $this->processPipes->readAndWrite(
                $running,
                '\\' !== \DIRECTORY_SEPARATOR || !$running
            );
            ...
            foreach ($output as $type => $data) {
                if (3 !== $type) {
                    $ready =
                        $callback(
                            self::STDOUT === $type ? self::OUT : self::ERR,
                            $data
                        ) || $ready;
            ...
            if ($yieldErr) {
                ...
                if (isset($err[0])) {
                    if ($clearOutput) {
                        ...
                        $this->incrementalErrorOutputOffset = ftell(
                            $this->stderr
                        );
                    }
    ...
    public function getExitCodeText()
    {
        ...
        return isset(self::$exitCodes[$exitcode])
            ? self::$exitCodes[$exitcode]
            : 'Unknown error';
    }
    ...
    public function getTermSignal()
    {
        ...
        if (
            ...
            -1 === $this->processInformation['termsig']
        ) {
            throw new RuntimeException(
    ...
    {
        return \is_array($this->commandline)
            ? implode(
                ' ',
                array_map([$this, 'escapeArgument'], $this->commandline)
            )
            : $this->commandline;
    }
    ...
    public function inheritEnvironmentVariables($inheritEnv = true)
    {
        if (!$inheritEnv) {
            throw new InvalidArgumentException(
                'Not inheriting environment variables is not supported.'
            );
        }
    ...
    public function checkTimeout()
    {
        ...
        if (
            null !== $this->timeout &&
            $this->timeout < microtime(true) - $this->starttime
        ) {
            $this->stop(0);
        ...
        if (
            ...
            $this->idleTimeout < microtime(true) - $this->lastOutputTime
        ) {
            $this->stop(0);
    ...
    private function getDescriptors(): array
    {
        ...
        if ('\\' === \DIRECTORY_SEPARATOR) {
            $this->processPipes = new WindowsPipes(
                $this->input,
                !$this->outputDisabled || $this->hasCallback
            );
        } else {
            $this->processPipes = new UnixPipes(
                $this->isTty(),
                $this->isPty(),
                $this->input,
                !$this->outputDisabled || $this->hasCallback
    ...
    protected function updateStatus($blocking)
    {
        ...
        if ($this->fallbackStatus && $this->isSigchildEnabled()) {
            $this->processInformation =
                $this->fallbackStatus + $this->processInformation;
        }
    ...
    private function doSignal(int $signal, bool $throwException): bool
    {
        ...
        if ('\\' === \DIRECTORY_SEPARATOR) {
            exec(
                sprintf('taskkill /F /T /PID %d 2>&1', $pid),
                $output,
                $exitCode
            );
            ...
            if (!$ok) {
                if ($throwException) {
                    throw new RuntimeException(
                        sprintf('Error while sending signal "%s".', $signal)
                    );
                }
