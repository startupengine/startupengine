...
class RouteCompiler implements RouteCompilerInterface
{
    ...
    private static function compilePattern(Route $route, $pattern, $isHost)
    {
        ...
        foreach ($matches as $match) {
            ...
            if (null === $regexp) {
                ...
                $nextSeparator = self::findNextSeparator(
                    $followingPattern,
                    $useUtf8
                );
                $regexp = sprintf(
                    ...
                    '' !== $nextSeparator
                        ? preg_quote($nextSeparator, self::REGEX_DELIMITER)
                        : ''
                );
                if (
                    ('' !== $nextSeparator &&
                        !preg_match('#^\{\w+\}#', $followingPattern)) ||
                    '' === $followingPattern
                ) {
                ...
                $regexp = self::transformCapturingGroupsToNonCapturings(
                    $regexp
                );
            }
     ...
     */
    private static function determineStaticPrefix(
        Route $route,
        array $tokens
    ): string {
        if ('text' !== $tokens[0][0]) {
        ...
        if (
            isset($tokens[1][1]) &&
            '/' !== $tokens[1][1] &&
            false === $route->hasDefault($tokens[1][3])
        ) {
            $prefix .= $tokens[1][1];
        }
     ...
     */
    private static function findNextSeparator(
        string $pattern,
        bool $useUtf8
    ): string {
        if ('' == $pattern) {
     ...
     */
    private static function computeRegexp(
        array $tokens,
        int $index,
        int $firstOptional
    ): string {
        ...
        if ('text' === $token[0]) {
            ...
            if (0 === $index && 0 === $firstOptional) {
                ...
                return sprintf(
                    '%s(?P<%s>%s)?',
                    preg_quote($token[1], self::REGEX_DELIMITER),
                    $token[3],
                    $token[2]
                );
            } else {
    ...
    private static function transformCapturingGroupsToNonCapturings(
        string $regexp
    ): string {
