...
class YamlFileLoader extends FileLoader
{
    ...
    public function load($file, $type = null)
    {
        ...
        if (!stream_is_local($path)) {
            throw new \InvalidArgumentException(
                sprintf('This is not a local file "%s".', $path)
            );
        }
        ...
        try {
            $parsedConfig = $this->yamlParser->parseFile(
                $path,
                Yaml::PARSE_CONSTANT
            );
        } catch (ParseException $e) {
            throw new \InvalidArgumentException(
                sprintf('The file "%s" does not contain valid YAML.', $path),
                0,
                $e
            );
        }
        ...
        if (!\is_array($parsedConfig)) {
            throw new \InvalidArgumentException(
                sprintf('The file "%s" must contain a YAML array.', $path)
            );
        }
    ...
    protected function parseRoute(
        ...
        $requirements = isset($config['requirements'])
            ? $config['requirements']
            : [];
    ...
    protected function validate($config, $name, $path)
    {
        ...
        if (
            ...
            isset($config['defaults']['_controller'])
        ) {
            throw new \InvalidArgumentException(
                sprintf(
