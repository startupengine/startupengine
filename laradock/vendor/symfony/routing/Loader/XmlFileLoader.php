...
class XmlFileLoader extends FileLoader
{
    ...
    protected function parseRoute(
        ...
        $schemes = preg_split(
            '/[\s,\|]++/',
            $node->getAttribute('schemes'),
            -1,
            PREG_SPLIT_NO_EMPTY
        );
        $methods = preg_split(
            '/[\s,\|]++/',
            $node->getAttribute('methods'),
            -1,
            PREG_SPLIT_NO_EMPTY
        );
    ...
    protected function parseImport(
        ...
        $host = $node->hasAttribute('host')
            ? $node->getAttribute('host')
            : null;
        $schemes = $node->hasAttribute('schemes')
            ? preg_split(
                '/[\s,\|]++/',
                $node->getAttribute('schemes'),
                -1,
            ...
            : null;
        $methods = $node->hasAttribute('methods')
            ? preg_split(
                '/[\s,\|]++/',
                $node->getAttribute('methods'),
                -1,
            ...
            : null;
        $trailingSlashOnRoot = $node->hasAttribute('trailing-slash-on-root')
            ? XmlUtils::phpize($node->getAttribute('trailing-slash-on-root'))
            : true;
    ...
    private function parseConfigs(\DOMElement $node, $path)
    {
        ...
        foreach (
            ...
            switch ($n->localName) {
                case 'path':
                    ...
                    break;
                case 'prefix':
                    $prefixes[$n->getAttribute('locale')] = trim(
                        $n->textContent
                    );
                    break;
                case 'default':
                    if ($this->isElementValueNull($n)) {
                        ...
                        $defaults[
                            $n->getAttribute('key')
                        ] = $this->parseDefaultsConfig($n, $path);
                    }

                    break;
                case 'requirement':
                    $requirements[$n->getAttribute('key')] = trim(
                        $n->textContent
                    );
                    break;
                case 'option':
        ...
        if (($controller = $node->getAttribute('controller'))) {
            if (isset($defaults['_controller'])) {
                $name = $node->hasAttribute('id')
                    ? sprintf('"%s"', $node->getAttribute('id'))
                    : sprintf('the "%s" tag', $node->tagName);
        ...
        switch ($node->localName) {
            ...
            case 'map':
                ...
                foreach ($node->childNodes as $element) {
                    ...
                    ] = $this->parseDefaultNode($element, $path);
                }
    ...
    private function isElementValueNull(\DOMElement $element)
    {
        ...
        return 'true' === $element->getAttributeNS($namespaceUri, 'nil') ||
