...
class UrlMatcherTest extends TestCase
{
    ...
    public function testMatch()
    {
        ...
        $collection->add(
            'bar',
            new Route('/{bar}/foo', ['bar' => 'bar'], ['bar' => 'foo|bar'])
        );
        $matcher = $this->getUrlMatcher($collection);
        $this->assertEquals(
            ['_route' => 'bar', 'bar' => 'bar'],
            $matcher->match('/bar/foo')
        );
        $this->assertEquals(
            ['_route' => 'bar', 'bar' => 'foo'],
        ...
        $collection->add(
            'bar',
            new Route('/{bar}', ['bar' => 'bar'], ['bar' => 'foo|bar'])
        );
        $matcher = $this->getUrlMatcher($collection);
        $this->assertEquals(
            ['_route' => 'bar', 'bar' => 'foo'],
        ...
        $collection->add(
            'bar',
            new Route('/{foo}/{bar}', ['foo' => 'foo', 'bar' => 'bar'], [])
        );
        $matcher = $this->getUrlMatcher($collection);
        $this->assertEquals(
            ['_route' => 'bar', 'foo' => 'foo', 'bar' => 'bar'],
            $matcher->match('/')
        );
        $this->assertEquals(
            ['_route' => 'bar', 'foo' => 'a', 'bar' => 'bar'],
            $matcher->match('/a')
        );
        $this->assertEquals(
            ['_route' => 'bar', 'foo' => 'a', 'bar' => 'b'],
            $matcher->match('/a/b')
        );
    }
    ...
    public function testMatchWithDynamicPrefix()
    {
        ...
        $matcher = $this->getUrlMatcher($collection);
        $this->assertEquals(
            ['_locale' => 'fr', '_route' => 'foo', 'foo' => 'foo'],
            $matcher->match('/fr/b/foo')
        );
    }
    ...
    public function testMatchNonAlpha()
    {
        $collection = new RouteCollection();
        $chars =
            '!"$%éà &\'()*+,./:;<=>@ABCDEFGHIJKLMNOPQRSTUVWXYZ\\[]^_`abcdefghijklmnopqrstuvwxyz{|}~-';
        $collection->add(
            'foo',
            new Route(
                '/{foo}/bar',
        ...
        $matcher = $this->getUrlMatcher($collection);
        $this->assertEquals(
            ['_route' => 'foo', 'foo' => $chars],
            $matcher->match('/' . rawurlencode($chars) . '/bar')
        );
        $this->assertEquals(
            ['_route' => 'foo', 'foo' => $chars],
            $matcher->match('/' . strtr($chars, ['%' => '%25']) . '/bar')
        );
    }
    ...
    public function testDefaultRequirementForOptionalVariables()
    {
        ...
        $matcher = $this->getUrlMatcher($coll);
        $this->assertEquals(
            ['page' => 'my-page', '_format' => 'xml', '_route' => 'test'],
    ...
    public function testOptionalVariableWithNoRealSeparator()
    {
        ...
        $this->assertEquals(
            ['what' => 'All', '_route' => 'test'],
            $matcher->match('/get')
        );
        $this->assertEquals(
            ['what' => 'Sites', '_route' => 'test'],
            $matcher->match('/getSites')
        );
    ...
    public function testDefaultRequirementOfVariable()
    {
        ...
        $this->assertEquals(
            ['page' => 'index', '_format' => 'mobile.html', '_route' => 'test'],
    ...
    public function testDefaultRequirementOfVariableDisallowsNextSeparator()
    {
        ...
        $coll->add(
            'test',
            new Route('/{page}.{_format}', [], ['_format' => 'html|xml'])
        );
        $matcher = $this->getUrlMatcher($coll);
    ...
    public function testRequestCondition()
    {
        ...
        $route = new Route('/foo/{bar}');
        $route->setCondition(
            'request.getBaseUrl() == "/sub/front.php" and request.getPathInfo() == "/foo/bar"'
        );
        $coll->add('foo', $route);
        $matcher = $this->getUrlMatcher(
            $coll,
    ...
    public function testDecodeOnce()
    {
        ...
        $matcher = $this->getUrlMatcher($coll);
        $this->assertEquals(
            ['foo' => 'bar%23', '_route' => 'foo'],
            $matcher->match('/foo/bar%2523')
        );
    }
    ...
    public function testWithHost()
    {
        ...
        $coll->add(
            'foo',
            new Route('/foo/{foo}', [], [], [], '{locale}.example.com')
        );

        $matcher = $this->getUrlMatcher(
            $coll,
            new RequestContext('', 'GET', 'en.example.com')
        );
        $this->assertEquals(
            ['foo' => 'bar', '_route' => 'foo', 'locale' => 'en'],
            $matcher->match('/foo/bar')
        );
    }
    ...
    public function testWithHostOnRouteCollection()
    {
        ...
        $coll->add(
            'bar',
            new Route('/bar/{foo}', [], [], [], '{locale}.example.net')
        );
        ...
        $this->assertEquals(
            ['foo' => 'bar', '_route' => 'bar', 'locale' => 'en'],
            $matcher->match('/bar/bar')
        );
    }
    ...
    public function testWithOutHostHostDoesNotMatch()
    {
        ...
        $matcher = $this->getUrlMatcher(
            $coll,
            new RequestContext('', 'GET', 'example.com')
        );
        $matcher->match('/foo/bar');
    }
    ...
    public function testHostPattern()
    {
        ...
        $coll->add(
            'a',
            new Route('/{app}/{action}/{unused}', [], [], [], '{host}')
        );
        ...
        $matcher = $this->getUrlMatcher(
            $coll,
            new RequestContext('', 'GET', 'foo')
        );
        $this->assertEquals(
            $expected,
            $matcher->match('/an_app/an_action/unused')
        );
    }
    ...
    public function testUtf8AndMethodMatching()
    {
        ...
        $coll->add(
            'a',
            new Route(
                '/admin/api/list/{shortClassName}/{id}.{_format}',
                [],
                [],
                ['utf8' => true],
                '',
        ...
        $this->assertEquals(
            'c',
            $matcher->match('/admin/api/package.json')['_route']
        );
    }
    ...
    public function testSlashAndVerbPrecedence()
    {
        ...
        $coll->add(
            'a',
            new Route(
                '/api/customers/{customerId}/contactpersons/',
                [],
                [],
                [],
                '',
        ...
        $this->assertEquals(
            $expected,
            $matcher->match('/api/customers/123/contactpersons')
        );
