...
class PhpGeneratorDumperTest extends TestCase
{
    ...
    public function testDumpWithRoutes()
    {
        ...
        $projectUrlGenerator = new \ProjectUrlGenerator(
            new RequestContext('/app.php')
        );

        $absoluteUrlWithParameter = $projectUrlGenerator->generate(
            'Test',
            ['foo' => 'bar'],
            UrlGeneratorInterface::ABSOLUTE_URL
        );
        $absoluteUrlWithoutParameter = $projectUrlGenerator->generate(
            'Test2',
            [],
            UrlGeneratorInterface::ABSOLUTE_URL
        );
        $relativeUrlWithParameter = $projectUrlGenerator->generate(
            'Test',
            ['foo' => 'bar'],
            UrlGeneratorInterface::ABSOLUTE_PATH
        );
        $relativeUrlWithoutParameter = $projectUrlGenerator->generate(
            'Test2',
            [],
            UrlGeneratorInterface::ABSOLUTE_PATH
        );
        ...
        $this->assertEquals(
            'http://localhost/app.php/testing/bar',
            $absoluteUrlWithParameter
        );
        $this->assertEquals(
            'http://localhost/app.php/testing2',
    ...
    public function testDumpWithSimpleLocalizedRoutes()
    {
        ...
        $this->routeCollection->add(
            'test.en',
            (new Route('/testing/is/fun'))
                ->setDefault('_locale', 'en')
                ->setDefault('_canonical_route', 'test')
        );
        $this->routeCollection->add(
            'test.nl',
            (new Route('/testen/is/leuk'))
                ->setDefault('_locale', 'nl')
                ->setDefault('_canonical_route', 'test')
        );
        ...
        $projectUrlGenerator = new \SimpleLocalizedProjectUrlGenerator(
            $context,
            null,
            'en'
        );
        ...
        $urlWithSpecifiedLocale = $projectUrlGenerator->generate('test', [
            '_locale' => 'nl'
        ]);
        ...
        $this->assertEquals(
            '/app.php/testing/is/fun',
            $projectUrlGenerator->generate('test.en')
        );
    ...
    public function testDumpWithRouteNotFoundLocalizedRoutes()
    {
        ...
        $projectUrlGenerator = new \RouteNotFoundLocalizedProjectUrlGenerator(
            new RequestContext('/app.php'),
            null,
            'pl_PL'
        );
    ...
    public function testDumpWithFallbackLocaleLocalizedRoutes()
    {
        $this->routeCollection->add(
            'test.en',
            (new Route('/testing/is/fun'))->setDefault(
                '_canonical_route',
                'test'
            )
        );
        $this->routeCollection->add(
            'test.nl',
            (new Route('/testen/is/leuk'))->setDefault(
                '_canonical_route',
                'test'
            )
        );
        $this->routeCollection->add(
            'test.fr',
            (new Route('/tester/est/amusant'))->setDefault(
                '_canonical_route',
                'test'
            )
        );
        ...
        $projectUrlGenerator = new \FallbackLocaleLocalizedProjectUrlGenerator(
            $context,
            null,
            null
        );
        ...
        $this->assertEquals(
            '/app.php/testen/is/leuk',
            $projectUrlGenerator->generate('test', ['_locale' => 'nl_BE'])
        );
    ...
    public function testDumpWithTooManyRoutes()
    {
        ...
        $projectUrlGenerator = new \ProjectLargeUrlGenerator(
            new RequestContext('/app.php')
        );
    ...
    public function testDumpWithoutRoutes()
    {
        ...
        $projectUrlGenerator = new \WithoutRoutesUrlGenerator(
            new RequestContext('/app.php')
        );
    ...
    public function testGenerateNonExistingRoute()
    {
        ...
        $projectUrlGenerator = new \NonExistingRoutesUrlGenerator(
            new RequestContext()
        );
    ...
    public function testDumpForRouteWithDefaults()
    {
        ...
        $projectUrlGenerator = new \DefaultRoutesUrlGenerator(
            new RequestContext()
        );
    ...
    public function testDumpWithSchemeRequirement()
    {
        $this->routeCollection->add(
            'Test1',
            new Route('/testing', [], [], [], '', ['ftp', 'https'])
        );

        file_put_contents(
            $this->testTmpFilepath,
            $this->generatorDumper->dump(['class' => 'SchemeUrlGenerator'])
        );
        include $this->testTmpFilepath;

        $projectUrlGenerator = new \SchemeUrlGenerator(
            new RequestContext('/app.php')
        );

        $absoluteUrl = $projectUrlGenerator->generate(
            'Test1',
            [],
            UrlGeneratorInterface::ABSOLUTE_URL
        );
        $relativeUrl = $projectUrlGenerator->generate(
            'Test1',
            [],
            UrlGeneratorInterface::ABSOLUTE_PATH
        );
        ...
        $projectUrlGenerator = new \SchemeUrlGenerator(
            new RequestContext('/app.php', 'GET', 'localhost', 'https')
        );
