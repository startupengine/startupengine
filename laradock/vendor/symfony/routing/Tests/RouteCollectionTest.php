...
class RouteCollectionTest extends TestCase
{
    public function testRoute()
    {
        ...
        $this->assertNull(
            $collection->get('bar'),
            '->get() returns null if a route does not exist'
        );
    }
    ...
    public function testAddCollection()
    {
        ...
        $collection2->add(
            'grandchild',
            ($grandchild = new Route('/grandchild'))
        );
    ...
    public function testAddCollectionWithResources()
    {
        ...
        $collection->addResource(
            ($foo = new FileResource(__DIR__ . '/Fixtures/foo.xml'))
        );
        $collection1 = new RouteCollection();
        $collection1->addResource(
            ($foo1 = new FileResource(__DIR__ . '/Fixtures/foo1.xml'))
        );
        $collection->addCollection($collection1);
        $this->assertEquals(
            [$foo, $foo1],
            $collection->getResources(),
            '->addCollection() merges resources'
        );
    }
    ...
    public function testAddDefaultsAndRequirementsAndOptions()
    {
        ...
        $this->assertEquals(
            ...
            $collection->get('foo')->getDefaults(),
        ...
        $this->assertEquals(
            ['placeholder' => '\d+'],
            $collection->get('foo')->getRequirements(),
    ...
    public function testAddPrefixOverridesDefaultsAndRequirements()
    {
        ...
        $collection->add(
            'bar',
            ($bar = new Route('/bar.{_format}', [], ['_format' => 'json']))
        );
        $collection->addPrefix('/admin', [], ['_format' => 'html']);

        $this->assertEquals(
            'html',
            $collection->get('foo')->getRequirement('_format'),
            '->addPrefix() overrides existing requirements'
        );
        $this->assertEquals(
            'html',
            $collection->get('bar')->getRequirement('_format'),
            '->addPrefix() overrides existing requirements'
        );
    }
    ...
    public function testResource()
    {
        ...
        $collection->addResource(
            ($bar = new FileResource(__DIR__ . '/Fixtures/bar.xml'))
        );
        $collection->addResource(
            new FileResource(__DIR__ . '/Fixtures/foo.xml')
        );
    ...
    public function testUniqueRouteWithGivenName()
    {
        ...
        $this->assertSame(
            $new,
            $collection1->get('foo'),
            '->get() returns new route that overrode previous one'
        );
        // size of 1 because collection1 contains /new but not /old anymore
        $this->assertCount(
            1,
            $collection1->getIterator(),
    ...
    public function testGet()
    {
        ...
        $this->assertSame(
            $b,
            $collection1->get('b'),
            '->get() returns correct route in child collection'
        );
        $this->assertSame(
            $c,
            $collection1->get('$péß^a|'),
            '->get() can handle special characters'
        );
        $this->assertNull(
            $collection2->get('a'),
            '->get() does not return the route defined in parent collection'
        );
        $this->assertNull(
            $collection1->get('non-existent'),
            '->get() returns null when route does not exist'
        );
        $this->assertNull(
            $collection1->get(0),
            '->get() does not disclose internal child RouteCollection'
        );
    }
    ...
    public function testAddNamePrefixCanonicalRouteName()
    {
        ...
        $collection->add(
            'foo',
            new Route('/foo', ['_canonical_route' => 'foo'])
        );
        $collection->add(
            'bar',
            new Route('/bar', ['_canonical_route' => 'bar'])
        );
        $collection->add(
            'api_foo',
            new Route('/api/foo', ['_canonical_route' => 'api_foo'])
        );
        $collection->addNamePrefix('api_');

        $this->assertEquals(
            'api_foo',
            $collection->get('api_foo')->getDefault('_canonical_route')
        );
        $this->assertEquals(
            'api_bar',
            $collection->get('api_bar')->getDefault('_canonical_route')
        );
        $this->assertEquals(
            'api_api_foo',
            $collection->get('api_api_foo')->getDefault('_canonical_route')
        );
    }
}
