...
class RouteCollectionBuilderTest extends TestCase
{
    ...
    {
        $resolvedLoader = $this->getMockBuilder(
            'Symfony\Component\Config\Loader\LoaderInterface'
        )->getMock();
        $resolver = $this->getMockBuilder(
        ...
        $expectedCollection->addResource(
            new FileResource(__DIR__ . '/Fixtures/file_resource.yml')
        );
    ...
    public function testAdd()
    {
        ...
        $addedRoute = $collectionBuilder->add(
            '/checkout',
            'AppBundle:Order:checkout'
        );
        $addedRoute2 = $collectionBuilder->add(
            '/blogs',
            'AppBundle:Blog:list',
            'blog_list'
        );
        ...
        $this->assertEquals(
            'AppBundle:Order:checkout',
            $addedRoute->getDefault('_controller')
        );
    ...
    public function testFlushOrdering()
    {
        ...
        $importedCollection->add(
            'imported_route1',
            new Route('/imported/foo1')
        );
        $importedCollection->add(
            'imported_route2',
            new Route('/imported/foo2')
        );
    ...
    public function testFlushSetsRouteNames()
    {
        ...
        // add a "named" route
        $collectionBuilder->add(
            '/admin',
            'AppBundle:Admin:dashboard',
            'admin_dashboard'
        );
        // add an unnamed route
        $collectionBuilder
            ->add('/blogs', 'AppBundle:Blog:list')
            ->setMethods(['GET']);
    ...
    public function testFlushSetsDetailsOnChildrenRoutes()
    {
        ...
        $routes
            ->add('/blogs/{page}', 'listAction', 'blog_list')
            ->setDefault('page', 1)
            ->setRequirement('id', '\d+')
        ...
        $this->assertEquals(
            'fr|en',
            $actualListRoute->getRequirement('_locale')
        );
     ...
     */
    public function testFlushPrefixesPaths(
        $collectionPrefix,
        $routePath,
        $expectedPath
    ) {
        ...
        $this->assertEquals(
            $expectedPath,
            $collection->get('test_route')->getPath()
        );
    }
    ...
    public function testFlushSetsPrefixedWithMultipleLevels()
    {
        ...
        $otherAdminRoutes->add(
            '/sales',
            'StatsController::indexAction',
            'admin_stats_sales'
        );
        ...
        $this->assertEquals(
            '/admin/dashboard',
            $collection->get('admin_dashboard')->getPath(),
            'Routes before mounting have the prefix'
        );
        $this->assertEquals(
            '/admin/users',
            $collection->get('admin_users')->getPath(),
            'Routes after mounting have the prefix'
        );
        $this->assertEquals(
            '/admin/blog/new',
            $collection->get('admin_blog_new')->getPath(),
            'Sub-collections receive prefix even if mounted before parent prefix'
        );
        $this->assertEquals(
            '/admin/stats/sales',
            $collection->get('admin_stats_sales')->getPath(),
            'Sub-collections receive prefix if mounted after parent prefix'
        );
        $this->assertEquals(
            '/admin/imported/foo',
            $collection->get('imported_route')->getPath(),
            'Normal RouteCollections are also prefixed properly'
        );
    }
    ...
    public function testAutomaticRouteNamesDoNotConflict()
    {
        ...
        // route 2
        $accountRoutes->add('/dashboard', '')->setMethods(['GET']);
        // route 3
        $accountRoutes->add('/dashboard', '')->setMethods(['POST']);
