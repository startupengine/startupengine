...
EOF;
    ...
    public function addExpressionLanguageProvider(
    ...
    private function generateProperties(): string
    {
        ...
        while (true) {
            try {
                $this->signalingException = new \RuntimeException(
                    'preg_match(): Compilation failed: regular expression is too large'
                );
                $code .= $this->compileDynamicRoutes(
                    $dynamicRoutes,
                    $matchHost,
                    $chunkLimit,
                    $conditions
...
EOF;
        ...
        foreach ($collection->all() as $name => $route) {
            ...
            if (($hasTrailingSlash = '/' !== $route->getPath())) {
                ...
                $regex = substr_replace(
                    $regex,
                    '/?$',
                    $pos - $hasTrailingSlash,
                    1 + $hasTrailingSlash
                );
            }

            if (!$compiledRoute->getPathVariables()) {
                $host = !$compiledRoute->getHostVariables()
                    ? $route->getHost()
                    : '';
                $url = $route->getPath();
                if ($hasTrailingSlash) {
                ...
                foreach ($dynamicRegex as list($hostRx, $rx, $prefix)) {
                    if (
                        ('' === $prefix || 0 === strpos($url, $prefix)) &&
                        preg_match($rx, $url) &&
                        (!$host || !$hostRx || preg_match($hostRx, $host))
                    ) {
     ...
     */
    private function compileStaticRoutes(
        array $staticRoutes,
        array &$conditions
    ): string {
        if (!$staticRoutes) {
        ...
        foreach ($staticRoutes as $url => $routes) {
            ...
            foreach ($routes as $name => list($route, $hasTrailingSlash)) {
                $code .= $this->compileRoute(
                    $route,
                    $name,
                    !$route->compile()->getHostVariables()
                        ? $route->getHost()
     ...
     */
    private function compileDynamicRoutes(
        ...
        foreach ($collection->all() as $name => $route) {
            ...
            if (
                ...
                ($prev !== $rx[0] && $route->compile()->getPathVariables())
            ) {
                $chunkSize = 1;
        ...
        foreach ($perModifiers as list($modifiers, $routes)) {
            ...
            foreach ($perHost as list($hostRegex, $routes)) {
                ...
                foreach ($routes->all() as $name => $route) {
                    ...
                    $hasTrailingVar = (bool) preg_match(
                        '#\{\w+\}/?$#',
                        $route->getPath()
                    );
                ...
                $code .= $this->compileStaticPrefixCollection(
                    $tree,
                    $state,
                    0,
                    $conditions
            ...
            set_error_handler(function ($type, $message) {
                throw 0 ===
                strpos($message, $this->signalingException->getMessage())
     ...
     */
    private function compileStaticPrefixCollection(
        StaticPrefixCollection $tree,
        \stdClass $state,
        int $prefixLen,
        array &$conditions
    ): string {
    ...
    private function compileRoute(
        ...
        if (($condition = $route->getCondition())) {
            $condition = $this->getExpressionLanguage()->compile($condition, [
                'context',
                'request'
            ]);
            $condition =
                ...
                ($conditions[$condition] =
                    (false !== strpos($condition, '$request') ? 1 : -1) *
                    \count($conditions));
        } else {
            $condition = 'null';
        }
        ...
        if (!\is_array($value)) {
            if (\is_object($value)) {
                throw new \InvalidArgumentException(
                    'Symfony\Component\Routing\Route cannot contain objects.'
                );
            }
