...
class StaticPrefixCollection
{
    ...
    public function addRoute(string $prefix, $route)
    {
        ...
        for ($i = \count($this->items) - 1; 0 <= $i; --$i) {
            ...
            list($commonPrefix, $commonStaticPrefix) = $this->getCommonPrefix(
                $prefix,
                $this->prefixes[$i]
            );
            ...
            if ($this->prefix === $commonPrefix) {
                ...
                if (
                    ...
                    $this->prefix !== $this->staticPrefixes[$i]
                ) {
                ...
                if (
                    $this->prefixes[$i] !== $this->staticPrefixes[$i] &&
                    $this->prefix === $this->staticPrefixes[$i]
                ) {
    ...
    private function getCommonPrefix(
        ...
        for (
            $i = $baseLength;
            $i < $end && $prefix[$i] === $anotherPrefix[$i];
            ++$i
        ) {
            if ('(' === $prefix[$i]) {
                ...
                for ($j = 1 + $i, $n = 1; $j < $end && 0 < $n; ++$j) {
                    ...
                    if ('(' === $prefix[$j]) {
                        ...
                        '\\' === $prefix[$j] &&
                        (++$j === $end || $prefix[$j] !== $anotherPrefix[$j])
                    ) {
                        --$j;
                        break;
                    }
        ...
        if (
            $i < $end &&
            0b10 === \ord($prefix[$i]) >> 6 &&
            preg_match('//u', $prefix . ' ' . $anotherPrefix)
        ) {
            do {
