...
class UrlMatcher implements UrlMatcherInterface, RequestMatcherInterface
{
    ...
    public function match($pathinfo)
    {
        ...
        throw 0 < \count($this->allow)
            ...
            : new ResourceNotFoundException(
                sprintf('No routes found for "%s".', $pathinfo)
            );
    }
    ...
    protected function matchCollection($pathinfo, RouteCollection $routes)
    {
        ...
        foreach ($routes as $name => $route) {
            ...
            if (
                '' !== $staticPrefix &&
                0 !== strpos($trimmedPathinfo, $staticPrefix)
            ) {
                continue;
            }
            ...
            $hasTrailingVar =
                $trimmedPathinfo !== $pathinfo &&
                preg_match('#\{\w+\}/?$#', $route->getPath());

            if (
                '/' !== $pathinfo &&
                !$hasTrailingVar &&
                $hasTrailingSlash === ($trimmedPathinfo === $pathinfo)
            ) {
                if (
                    $supportsTrailingSlash &&
                    (!$requiredMethods || \in_array('GET', $requiredMethods))
                ) {
            ...
            if (
                $hasTrailingSlash &&
                $hasTrailingVar &&
                preg_match($regex, $trimmedPathinfo, $m)
            ) {
                $matches = $m;
            }
            ...
            $hasRequiredScheme =
                !$route->getSchemes() ||
                $route->hasScheme($this->context->getScheme());
            if ($requiredMethods) {
    ...
    protected function handleRouteRequirements($pathinfo, $name, Route $route)
    {
        ...
        if (
            $route->getCondition() &&
            !$this->getExpressionLanguage()->evaluate($route->getCondition(), [
