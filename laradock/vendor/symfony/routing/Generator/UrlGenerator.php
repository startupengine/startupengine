    ...
    ConfigurableRequirementsInterface
{
    ...
    protected function doGenerate(
            ...
            if ('variable' === $token[0]) {
                if (
                    ...
                    (null !== $mergedParams[$token[3]] &&
                        (string) $mergedParams[$token[3]] !==
                            (string) $defaults[$token[3]])
                ) {
                    // check requirement (while ignoring look-around patterns)
                    if (
                        null !== $this->strictRequirements &&
                        !preg_match(
                            '#^' .
                                preg_replace(
                                    '/\(\?(?:=|<=|!|<!)((?:[^()\\\\]+|\\\\.|\((?1)\))*)\)/',
        ...
        if ($hostTokens) {
            ...
            foreach ($hostTokens as $token) {
                if ('variable' === $token[0]) {
                    ...
                    $routeHost =
                        $token[1] . $mergedParams[$token[3]] . $routeHost;
                } else {
        ...
        if (
            (self::ABSOLUTE_URL === $referenceType ||
            ...
            if ('http' === $scheme && 80 != $this->context->getHttpPort()) {
                ...
                'https' === $scheme &&
                443 != $this->context->getHttpsPort()
            ) {
                $port = ':' . $this->context->getHttpsPort();
            }

            $schemeAuthority =
                self::NETWORK_PATH === $referenceType ? '//' : "$scheme://";
        ...
        if ('' !== $fragment) {
            $url .=
                '#' .
                strtr(rawurlencode($fragment), ['%2F' => '/', '%3F' => '?']);
        }
    ...
    public static function getRelativePath($basePath, $targetPath)
    {
        ...
        $targetDirs = explode(
            '/',
            isset($targetPath[0]) && '/' === $targetPath[0]
                ? substr($targetPath, 1)
                : $targetPath
        );
