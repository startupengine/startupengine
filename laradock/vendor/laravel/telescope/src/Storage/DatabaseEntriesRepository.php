    ...
    TerminableRepository
{
    ...
    {
        $this->table('telescope_entries')->insert(
            $exceptions
    ...
    protected function updateTags($entry)
    {
        if (!empty($entry->tagsChanges['added'])) {
            $this->table('telescope_entries_tags')->insert(
                collect($entry->tagsChanges['added'])
                    ->map(function ($tag) use ($entry) {
                        return [
        ...
        collect($entry->tagsChanges['removed'])->each(function ($tag) use (
            $entry
        ) {
    ...
    {
        return $this->table('telescope_monitoring')
            ->pluck('tag')
            ->all();
    }
