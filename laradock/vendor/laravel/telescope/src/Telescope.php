...
class Telescope
{
     ...
     * @var array
     */
    public static $hiddenRequestHeaders = ['authorization'];
    ...
    public static function store(EntriesRepository $storage)
    {
        ...
        if (
            !collect(static::$filterBatchUsing)->every->__invoke(
                collect(static::$entriesQueue)
            )
        ) {
            static::flushEntries();
        }
    ...
    {
        return collect(static::$entriesQueue)->each(function ($entry) use (
            $batchId
        ) {
    ...
    {
        return collect(static::$updatesQueue)->each(function ($entry) use (
            $batchId
        ) {
