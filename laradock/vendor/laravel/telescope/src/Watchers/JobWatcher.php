...
class JobWatcher extends Watcher
{
    ...
    public function register($app)
    {
        Queue::createPayloadUsing(function ($connection, $queue, $payload) {
            return [
                'telescope_uuid' => optional(
                    $this->recordJob($connection, $queue, $payload)
                )->uuid
            ];
        });

        $app['events']->listen(JobProcessed::class, [
            $this,
            'recordProcessedJob'
        ]);
    ...
    public function recordJob($connection, $queue, array $payload)
    {
        ...
        Telescope::recordJob(
            ($entry = IncomingEntry::make($content)->tags(
                $this->tags($payload)
            ))
        );
    ...
    public function recordProcessedJob(JobProcessed $event)
    {
        ...
        Telescope::recordUpdate(
            EntryUpdate::make($uuid, EntryType::JOB, ['status' => 'processed'])
        );
    }
    ...
    public function recordFailedJob(JobFailed $event)
    {
        ...
        Telescope::recordUpdate(
            EntryUpdate::make($uuid, EntryType::JOB, [
                'status' => 'failed',
                'exception' => [
     ...
     * @return array
     */
    protected function defaultJobData(
        $connection,
        $queue,
        array $payload,
        array $data
