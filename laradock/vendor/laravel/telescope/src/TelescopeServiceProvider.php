...
class TelescopeServiceProvider extends ServiceProvider
{
    ...
    private function registerPublishing()
    {
        if ($this->app->runningInConsole()) {
            $this->publishes(
                [
                    __DIR__ . '/Storage/migrations' => database_path(
                        'migrations'
                    )
                ],
            ...
            $this->publishes(
                [
                    __DIR__ . '/../config/telescope.php' => config_path(
                        'telescope.php'
                    )
                ],
                'telescope-config'
            ...
            $this->publishes(
                [
                    __DIR__ .
                        '/../stubs/TelescopeServiceProvider.stub' => app_path(
    ...
    protected function registerStorageDriver()
    {
        ...
        if (
            method_exists(
                $this,
                ($method = 'register' . ucfirst($driver) . 'Driver')
            )
        ) {
            $this->$method();
        }
    ...
    protected function registerDatabaseDriver()
    {
        ...
        $this->app
            ->when(DatabaseEntriesRepository::class)
            ->needs('$connection')
    ...
    {
        return Telescope::$runsMigrations &&
