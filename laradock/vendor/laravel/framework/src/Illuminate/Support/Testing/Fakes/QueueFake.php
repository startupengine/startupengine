...
class QueueFake extends QueueManager implements Queue
{
    ...
    {
        return $this->assertPushed($job, function ($job, $pushedQueue) use (
            $callback,
            $queue
        ) {
     ...
     * @return void
     */
    public function assertPushedWithChain(
        $job,
        $expectedChain = [],
        $callback = null
    ) {
        ...
        $this->isChainOfObjects($expectedChain)
            ? $this->assertPushedWithChainOfObjects(
                $job,
                $expectedChain,
                $callback
            )
            : $this->assertPushedWithChainOfClasses(
                $job,
                $expectedChain,
                $callback
            );
     ...
     * @return void
     */
    protected function assertPushedWithChainOfObjects(
        $job,
        $expectedChain,
        $callback
    ) {
        $chain = collect($expectedChain)
            ->map(function ($job) {
    ...
    protected function assertPushedWithChainOfClasses(
        ...
        $matching = $this->pushed($job, $callback)
            ->map->chained->map(function ($chain) {
    ...
    public function pushed($job, $callback = null)
    {
        ...
        return collect($this->jobs[$job])
            ->filter(function ($data) use ($callback) {
