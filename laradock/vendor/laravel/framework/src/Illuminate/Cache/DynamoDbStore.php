...
class DynamoDbStore implements Store
{
    ...
    public function many(array $keys)
    {
        ...
        return array_merge(
            collect(array_flip($keys))
                ->map(function () {
                    return null;
                })
                ->all(),
            collect($response['Responses'][$this->table])
    ...
    protected function isExpired(array $item, $expiration = null)
    {
        ...
        return isset($item[$this->expirationAttribute]) &&
            $expiration->getTimestamp() >=
                $item[$this->expirationAttribute]['N'];
    }
                        ...
                        return [
                            'PutRequest' => [
                                'Item' => [
                                    ...
                                    $this->valueAttribute => [
                                        $this->type($value) => $this->serialize(
                                            $value
                                        )
                                    ],
                                    $this->expirationAttribute => [
    ...
    public function increment($key, $value = 1)
    {
        try {
            $response = $this->dynamo->updateItem([
                ...
                'ConditionExpression' =>
                    'attribute_exists(#key) AND #expires_at > :now',
