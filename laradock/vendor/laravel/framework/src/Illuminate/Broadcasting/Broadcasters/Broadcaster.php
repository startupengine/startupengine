...
abstract class Broadcaster implements BroadcasterContract
{
    ...
    protected function verifyUserCanAccessChannel($request, $channel)
    {
        foreach ($this->channels as $pattern => $callback) {
            ...
            $parameters = $this->extractAuthParameters(
                $pattern,
                $channel,
                $callback
            );
            ...
            if (
                ($result = $handler(
                    $this->retrieveUser($request, $channel),
                    ...$parameters
                ))
            ) {
    ...
    protected function resolveBinding($key, $value, $callbackParameters)
    {
        ...
        return $newValue === $value
            ? $this->resolveImplicitBindingIfPossible(
                $key,
                $value,
                $callbackParameters
            )
     ...
     * @throws \Symfony\Component\HttpKernel\Exception\AccessDeniedHttpException
     */
    protected function resolveImplicitBindingIfPossible(
        $key,
        $value,
        $callbackParameters
    ) {
    ...
    protected function binder()
    {
        if (!$this->bindingRegistrar) {
            $this->bindingRegistrar = Container::getInstance()->bound(
