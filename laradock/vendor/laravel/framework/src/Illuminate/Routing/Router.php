...
class Router implements RegistrarContract, BindingRegistrar
{
    ...
    public function resource($name, $controller, array $options = [])
    {
        if (
            $this->container &&
            $this->container->bound(ResourceRegistrar::class)
        ) {
    ...
    {
        return trim(
            trim($this->getLastGroupPrefix(), '/') . '/' . trim($uri, '/'),
            '/'
        ) ?:
            '/';
    }
    ...
    {
        $route->where(
            array_merge($this->patterns, $route->getAction()['where'] ?? [])
        );
    ...
    {
        $route = tap($this->routes->getByName($name))->bind(
            $this->currentRequest
        );
    ...
    public function substituteBindings($route)
    {
        foreach ($route->parameters() as $key => $value) {
            if (isset($this->binders[$key])) {
                $route->setParameter(
                    $key,
                    $this->performBinding($key, $value, $route)
                );
            }
        }
    ...
    {
        if (
            isset($this->middlewareGroups[$group]) &&
            !in_array($middleware, $this->middlewareGroups[$group])
        ) {
    ...
    public function model($key, $class, Closure $callback = null)
    {
        $this->bind(
            $key,
            RouteBinding::forModel($this->container, $class, $callback)
        );
    }
    ...
    public function auth(array $options = [])
    {
        ...
        $this->get('login', 'Auth\LoginController@showLoginForm')->name(
            'login'
        );
    ...
    {
        $this->get('email/verify', 'Auth\VerificationController@show')->name(
            'verification.notice'
        );
        $this->get(
            'email/verify/{id}',
        ...
        $this->get('email/resend', 'Auth\VerificationController@resend')->name(
            'verification.resend'
        );
    }
