...
trait ValidatesAttributes
{
    ...
    protected function checkDateTimeOrder($format, $first, $second, $operator)
    {
        ...
        if (
            !($secondDate = $this->getDateTimeWithOptionalFormat(
                $format,
                $second
            ))
        ) {
    ...
    {
        return $this->validateSame($attribute, $value, [
    ...
    public function validateDimensions($attribute, $value, $parameters)
    {
        if (
            ...
            $value->getClientMimeType() === 'image/svg+xml'
        ) {
            return true;
        }
    ...
    {
        return (isset($parameters['width']) &&
            $parameters['width'] != $width) ||
            (isset($parameters['min_width']) &&
                $parameters['min_width'] > $width) ||
            (isset($parameters['max_width']) &&
                $parameters['max_width'] < $width) ||
            (isset($parameters['height']) &&
                $parameters['height'] != $height) ||
            (isset($parameters['min_height']) &&
                $parameters['min_height'] > $height) ||
            (isset($parameters['max_height']) &&
    ...
    protected function getDistinctValues($attribute)
    {
        ...
        if (!array_key_exists($attributeName, $this->distinctValues)) {
            $this->distinctValues[
                $attributeName
            ] = $this->extractDistinctValues($attributeName);
        }
    ...
    protected function extractDistinctValues($attribute)
    {
        ...
        return Arr::where(Arr::dot($attributeData), function (
            $value,
            $key
        ) use ($pattern) {
     ...
     * @return int
     */
    protected function getExistCount(
        $connection,
        $table,
        $column,
        $value,
    ...
    public function validateGt($attribute, $value, $parameters)
    {
        ...
        if (
            ...
            (is_numeric($value) && is_numeric($parameters[0]))
        ) {
        ...
        return $this->getSize($attribute, $value) >
            $this->getSize($attribute, $comparedToValue);
    }
    ...
    {
        return $this->validateMimes($attribute, $value, [
            'jpeg',
            'png',
            'gif',
            'bmp',
    ...
    public function validateInArray($attribute, $value, $parameters)
    {
        ...
        $explicitPath = ValidationData::getLeadingExplicitAttributePath(
            $parameters[0]
        );
        ...
        $otherValues = Arr::where(Arr::dot($attributeData), function (
            $value,
            $key
        ) use ($parameters) {
    ...
    protected function shouldBlockPhpUpload($value, $parameters)
    {
        ...
        return $value instanceof UploadedFile
            ? in_array(
                trim(strtolower($value->getClientOriginalExtension())),
                $phpExtensions
            )
            : in_array(
                trim(strtolower($value->getExtension())),
                $phpExtensions
            );
    }
