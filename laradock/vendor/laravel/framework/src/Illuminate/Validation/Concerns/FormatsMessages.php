...
trait FormatsMessages
{
    ...
    protected function getCustomMessageFromTranslator($key)
    {
        ...
        return $this->getWildcardCustomMessages(
            Arr::dot((array) $this->translator->trans('validation.custom')),
            $shortKey,
            $key
        );
    }
    ...
    protected function getWildcardCustomMessages($messages, $search, $default)
    {
        foreach ($messages as $key => $message) {
            if (
                $search === $key ||
                (Str::contains($key, ['*']) && Str::is($key, $search))
            ) {
                return $message;
            }
        }
     ...
     * @return string|null
     */
    protected function callReplacer(
        $message,
        $attribute,
        $rule,
        $parameters,
        ...
        if ($callback instanceof Closure) {
            ...
            return $this->callClassBasedReplacer(
                $callback,
                $message,
                $attribute,
                $rule,
