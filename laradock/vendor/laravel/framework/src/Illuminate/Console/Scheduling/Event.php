...
class Event
{
    ...
    public function emailOutputTo($addresses, $onlyIfOutputExists = false)
    {
        ...
        return $this->then(function (Mailer $mailer) use (
            $addresses,
            $onlyIfOutputExists
        ) {
    ...
    protected function ensureOutputIsBeingCaptured()
    {
        if (
            ...
            $this->sendOutputTo(
                storage_path(
                    'logs/schedule-' . sha1($this->mutexName()) . '.log'
                )
            );
        }
    }
    ...
    {
        $this->environments = is_array($environments)
            ? $environments
            : func_get_args();
    ...
    public function nextRunDate(
        ...
        return Date::instance(
            CronExpression::factory($this->getExpression())->getNextRunDate(
                $currentTime,
                $nth,
                $allowCurrentDate,
                $this->timezone
