...
class LogManager implements LoggerInterface
{
    ...
    protected function get($name)
    {
        try {
            ...
            return tap($this->createEmergencyLogger(), function ($logger) use (
                $e
            ) {
                $logger->emergency(
    ...
    protected function tap($name, Logger $logger)
    {
        foreach ($this->configurationFor($name)['tap'] ?? [] as $tap) {
            ...
            $this->app
                ->make($class)
                ->__invoke($logger, ...explode(',', $arguments));
        }
    ...
    {
        $factory = is_callable(($via = $config['via']))
            ? $via
            : $this->app->make($via);
    ...
    {
        $handlers = collect($config['channels'])
    ...
    {
        return tap(new LineFormatter(null, null, true, true), function (
            $formatter
        ) {
