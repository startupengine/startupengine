...
class SQLiteGrammar extends Grammar
{
    ...
    public function compileSelect(Builder $query)
    {
        ...
        if ($query->unions) {
            $sql =
                'select * from (' . $sql . ') ' . $this->compileUnions($query);
        }
    ...
    public function compileUpdate(Builder $query, $values)
    {
        ...
        $columns = collect($values)
            ->map(function ($value, $key) use ($query) {
                return $this->wrap(Str::after($key, $query->from . '.')) .
                    ' = ' .
        ...
        if (isset($query->joins) || isset($query->limit)) {
            $selectSql = parent::compileSelect(
                $query->select("{$query->from}.rowid")
            );
        ...
        return trim(
            "update {$table} set {$columns} {$this->compileWheres($query)}"
        );
    }
