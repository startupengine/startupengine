...
class ChangeColumn
{
    ...
    protected static function getChangedDiff(
        ...
        $current = $schema->listTableDetails(
    ...
    protected static function getTableWithColumnChanges(
        ...
        foreach ($blueprint->getChangedColumns() as $fluent) {
            ...
            foreach ($fluent->getAttributes() as $key => $value) {
                if (
                    !is_null(
                        ($option = static::mapFluentOptionToDoctrine($key))
                    )
                ) {
                    if (
                        method_exists(
                        ...
                        $column->$method(
                            static::mapFluentValueToDoctrine($option, $value)
                        );
                    }
                }
            }
    ...
    protected static function getDoctrineColumnChangeOptions(Fluent $fluent)
    {
        ...
        if (in_array($fluent['type'], ['text', 'mediumText', 'longText'])) {
            $options['length'] = static::calculateDoctrineTextLength(
                $fluent['type']
            );
        }
