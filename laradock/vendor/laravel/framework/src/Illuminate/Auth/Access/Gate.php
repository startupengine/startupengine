...
class Gate implements GateContract
{
    ...
    public function define($ability, $callback)
    {
        if (is_callable($callback)) {
            ...
            $this->abilities[$ability] = $this->buildAbilityCallback(
                $ability,
                $callback
            );
        } else {
    ...
    {
        return collect($abilities)->contains(function ($ability) use (
            $arguments
        ) {
            return $this->check($ability, $arguments);
        });
    ...
    protected function methodAllowsGuests($class, $method)
    {
        ...
        if ($method) {
            ...
            return isset($parameters[0]) &&
                $this->parameterAllowsGuests($parameters[0]);
        }
    ...
    protected function resolveAuthCallback($user, $ability, array $arguments)
    {
        ...
        if (isset($this->stringCallbacks[$ability])) {
            [$class, $method] = Str::parseCallback(
                $this->stringCallbacks[$ability]
            );

            if (
                $this->canBeCalledWithUser($user, $class, $method ?: '__invoke')
            ) {
    ...
    protected function guessPolicyName($class)
    {
        if ($this->guessPolicyNamesUsingCallback) {
            return Arr::wrap(
                call_user_func($this->guessPolicyNamesUsingCallback, $class)
            );
        }
     ...
     */
    protected function resolvePolicyCallback(
        $user,
        $ability,
        array $arguments,
        $policy
     ...
     * @return mixed
     */
    protected function callPolicyMethod(
        $policy,
        $method,
        $user,
        array $arguments
