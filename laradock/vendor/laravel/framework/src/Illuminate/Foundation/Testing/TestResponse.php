...
class TestResponse
{
    ...
    public function assertCookie(
        ...
        $actual = $encrypted
            ? app('encrypter')->decrypt($cookieValue, $unserialize)
            : $cookieValue;
    ...
    public function assertSeeTextInOrder(array $values)
    {
        PHPUnit::assertThat(
            $values,
            new SeeInOrder(strip_tags($this->getContent()))
        );
    ...
    {
        $actual = json_encode(
            Arr::sortRecursive((array) $this->decodeResponseJson())
        );
    ...
    public function assertSessionDoesntHaveErrors(
        ...
        foreach ($keys as $key => $value) {
            if (is_int($key)) {
                PHPUnit::assertFalse(
                    $errors->has($value),
                    "Session has unexpected error: $value"
                );
            } else {
