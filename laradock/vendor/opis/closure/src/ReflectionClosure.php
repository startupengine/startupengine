...
class ReflectionClosure extends ReflectionFunction
{
    ...
    public function isStatic()
    {
        if ($this->isStaticClosure === null) {
            $this->isStaticClosure =
                strtolower(substr($this->getCode(), 0, 6)) === 'static';
        }
            ...
            switch ($state) {
                case 'start':
                    if ($token[0] === T_FUNCTION || $token[0] === T_STATIC) {
                        $code .= $token[1];
                        $state =
                            $token[0] === T_FUNCTION ? 'function' : 'static';
                    }
                    break;
                case 'static':
                    if (
                        ...
                        case T_DOUBLE_COLON:
                            if ($id_start !== '\\') {
                                if (
                                    ...
                                    !(
                                        $php7 &&
                                        in_array($id_start_ci, $php7_types)
                                    )
                                ) {
                                    if ($classes === null) {
                                        $classes = $this->getClasses();
    ...
    protected function getFileTokens()
    {
        ...
        if (!isset(static::$files[$key])) {
            static::$files[$key] = token_get_all(
                ...
                case 'use-group':
                    switch ($token[0]) {
                        ...
                        case '}':
                            ...
                            if ($alias !== '') {
                                if ($isFunc) {
                                    $functions[strtolower($alias)] =
                                        $prefix . $name;
                                } elseif ($isConst) {
                                    $constants[$alias] = $prefix . $name;
                                } else {
                                    $classes[strtolower($alias)] =
                                        $prefix . $name;
                                }
                            }
                            $name = $alias = '';
