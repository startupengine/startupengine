...
class SerializableClosure implements Serializable
{
    ...
    public function getReflector()
    {
        if ($this->reflector === null) {
            $this->reflector = new ReflectionClosure(
                $this->closure,
                $this->code
            );
            $this->code = null;
        ...
        if (static::$securityProvider !== null) {
            if ($data[0] !== '@') {
                throw new SecurityException(
                    "The serialized closure is not signed. " .
            ...
            throw new SecurityException(
                "The serialized closure is signed. " .
                    "Make sure you use a security provider for both serialization and unserialization."
            );
        }
     ...
     */
    public static function addSecurityProvider(
     ...
     */
    public static function wrapClosures(
        &$data,
     ...
     */
    public static function unwrapClosures(
        &$data,
        SplObjectStorage $storage = null
    ) {
        if ($storage === null) {
        ...
        if ($data instanceof static) {
            ...
            foreach ($data as $key => &$value) {
                if ($key === self::ARRAY_RECURSIVE_KEY) {
                    ...
                    $value instanceof SelfReference &&
                    $value->hash === $this->code['self']
                ) {
                ...
                foreach ($reflection->getProperties() as $property) {
                    ...
                    if (
                        ...
                        ($item instanceof SelfReference &&
                            $item->hash === $this->code['self'])
                    ) {
                        $this->code['objects'][] = array(
                            'instance' => $data,
                            'property' => $property,
                            'object' =>
                                $item instanceof SelfReference ? $this : $item
                        );
