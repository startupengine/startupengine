...
class MountManager implements FilesystemInterface
{
    ...
    public function mountFilesystem($prefix, FilesystemInterface $filesystem)
    {
        if (!is_string($prefix)) {
            throw new InvalidArgumentException(
                __METHOD__ . ' expects argument #1 to be a string.'
            );
        }
    ...
    public function getFilesystem($prefix)
    {
        if (!isset($this->filesystems[$prefix])) {
            throw new FilesystemNotFoundException(
                'No filesystem mounted with prefix ' . $prefix
            );
        }
    ...
    public function copy($from, $to, array $config = [])
    {
        ...
        $result = $this->getFilesystem($prefixTo)->writeStream(
            $to,
            $buffer,
            $config
        );
    ...
    public function move($from, $to, array $config = [])
    {
        ...
        if ($prefixFrom === $prefixTo) {
            ...
            if ($renamed && isset($config['visibility'])) {
                return $filesystem->setVisibility(
                    $pathTo,
                    $config['visibility']
                );
            }
    ...
    protected function getPrefixAndPath($path)
    {
        if (strpos($path, '://') < 1) {
            throw new InvalidArgumentException(
                'No prefix detected in path: ' . $path
            );
        }
    ...
    public function writeStream($path, $resource, array $config = [])
    {
        ...
        return $this->getFilesystem($prefix)->writeStream(
            $path,
            $resource,
            $config
        );
    ...
    public function updateStream($path, $resource, array $config = [])
    {
        ...
        return $this->getFilesystem($prefix)->updateStream(
            $path,
            $resource,
            $config
        );
    ...
    public function putStream($path, $resource, array $config = [])
    {
        ...
        return $this->getFilesystem($prefix)->putStream(
            $path,
            $resource,
            $config
        );
