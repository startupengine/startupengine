...
class Local extends AbstractAdapter
{
    ...
    public function __construct(
        ...
        if (!is_dir($root) || !is_readable($root)) {
            throw new LogicException(
                'The root path ' . $root . ' is not readable.'
            );
        }
    ...
    protected function ensureDirectory($root)
    {
        if (!is_dir($root)) {
            ...
            if (!is_dir($root)) {
                $errorMessage = isset($mkdirError['message'])
                    ? $mkdirError['message']
                    : '';
                throw new Exception(
                    sprintf(
    ...
    public function writeStream($path, $resource, Config $config)
    {
        ...
        if (
            !$stream ||
            stream_copy_to_stream($resource, $stream) === false ||
            !fclose($stream)
        ) {
            return false;
        }
    ...
    public function getMimetype($path)
    {
        ...
        if (
            in_array($mimetype, ['application/octet-stream', 'inode/x-empty'])
        ) {
    ...
    public function createDir($dirname, Config $config)
    {
        ...
        if (
            !is_dir($location) &&
            !mkdir($location, $this->permissionMap['dir'][$visibility], true)
        ) {
            $return = false;
        } else {
    ...
    public function deleteDir($dirname)
    {
        ...
        $contents = $this->getRecursiveDirectoryIterator(
            $location,
