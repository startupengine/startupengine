...
class Ftp extends AbstractFtpAdapter
{
    ...
    {
        if (
            is_bool($this->ignorePassiveAddress) &&
            defined('FTP_USEPASVADDRESS')
        ) {
            ftp_set_option(
                $this->connection,
                FTP_USEPASVADDRESS,
                !$this->ignorePassiveAddress
            );
        }
    ...
    protected function setConnectionRoot()
    {
        ...
        if ($root && !ftp_chdir($connection, $root)) {
            throw new RuntimeException(
                'Root is invalid or does not exist: ' . $this->getRoot()
            );
        }
    ...
    public function deleteDir($dirname)
    {
        ...
        $contents = array_reverse(
            $this->listDirectoryContents($dirname, false)
        );
    ...
    public function createDir($dirname, Config $config)
    {
        ...
        foreach ($directories as $directory) {
            if (
                false === $this->createActualDirectory($directory, $connection)
            ) {
                $this->setConnectionRoot();
    ...
    public function isConnected()
    {
        try {
            return is_resource($this->connection) &&
                ftp_rawlist($this->connection, $this->getRoot()) !== false;
