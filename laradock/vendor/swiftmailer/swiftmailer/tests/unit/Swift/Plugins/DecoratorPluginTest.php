...
class Swift_Plugins_DecoratorPluginTest extends \SwiftMailerTestCase
{
    ...
    public function testReplacementsCanBeMadeInHeaders()
    {
        $headers = $this->createHeaders([
            ($returnPathHeader = $this->createHeader(
                'Return-Path',
                'foo-{id}@swiftmailer.org'
            )),
            ($toHeader = $this->createHeader(
                'Subject',
                'A message for {name}!'
            ))
        ]);
        ...
        $message->shouldReceive('setBody')->zeroOrMoreTimes();
        $toHeader->shouldReceive('setFieldBodyModel')->zeroOrMoreTimes();
        $returnPathHeader
            ->shouldReceive('setFieldBodyModel')
            ->zeroOrMoreTimes();
    ...
    public function testReplacementsAreMadeOnSubparts()
    {
        ...
        $part2 = $this->createPart(
            'text/html',
            'Your <em>name</em> is {name}?',
            '2@x'
        );
        $message = $this->createMessage(
            $this->createHeaders(),
        ...
        $part1->shouldReceive('setBody')->zeroOrMoreTimes();
        $part2->shouldReceive('setBody')->zeroOrMoreTimes();
    ...
    public function testReplacementsCanBeTakenFromCustomReplacementsObject()
    {
        ...
        $message->shouldReceive('setBody')->zeroOrMoreTimes();
        $replacements
            ->shouldReceive('getReplacementsFor')
            ->once()
            ->with('foo@bar')
            ->andReturn(['{a}' => 'b']);
        $replacements
