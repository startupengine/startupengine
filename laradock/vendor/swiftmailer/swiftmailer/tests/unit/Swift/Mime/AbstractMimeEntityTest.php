...
require_once dirname(dirname(dirname(__DIR__))) .
    '/fixtures/MimeEntityFixture.php';
...
abstract class Swift_Mime_AbstractMimeEntityTest extends \SwiftMailerTestCase
{
    ...
    public function testContentTypeIsSetInHeader()
    {
        ...
        $ctype
            ->shouldReceive('setFieldBodyModel')
            ->once()
            ->with('image/jpeg');
        $ctype
            ->shouldReceive('setFieldBodyModel')
    ...
    public function testContentTypeHeaderIsAddedIfNoneSet()
    {
        ...
        $headers
            ->shouldReceive('addParameterizedHeader')
            ->once()
    ...
    public function testSettingEncoderUpdatesTransferEncoding()
    {
        ...
        $encoding
            ...
            ->once()
            ->with('base64');
        $encoding->shouldReceive('setFieldBodyModel')->zeroOrMoreTimes();
    ...
    public function testIdIsSetInHeader()
    {
        ...
        $cid
            ...
            ->once()
            ->with('foo@bar');
        $cid->shouldReceive('setFieldBodyModel')->zeroOrMoreTimes();
    ...
    {
        for (
            $level = Swift_Mime_SimpleMimeEntity::LEVEL_MIXED;
            $level > Swift_Mime_SimpleMimeEntity::LEVEL_TOP;
            $level /= 2
        ) {
            ...
            $cType->shouldReceive('setFieldBodyModel')->zeroOrMoreTimes();
    ...
    {
        for (
            $level = Swift_Mime_SimpleMimeEntity::LEVEL_ALTERNATIVE;
            $level > Swift_Mime_SimpleMimeEntity::LEVEL_MIXED;
            $level /= 2
        ) {
            $child = $this->createChild($level);
    ...
    {
        for (
            $level = Swift_Mime_SimpleMimeEntity::LEVEL_RELATED;
            $level > Swift_Mime_SimpleMimeEntity::LEVEL_ALTERNATIVE;
            $level /= 2
        ) {
            $child = $this->createChild($level);
     ...
     */
        ...
        $child1 = new MimeEntityFixture(
            Swift_Mime_SimpleMimeEntity::LEVEL_ALTERNATIVE,
            "Content-Type: text/plain\r\n" . "\r\n" . 'foobar',
            'text/plain'
        );

        $child2 = new MimeEntityFixture(
            Swift_Mime_SimpleMimeEntity::LEVEL_ALTERNATIVE,
    ...
    public function testMixingLevelsIsHierarchical()
    {
        ...
        $part = $this->createChild(
            Swift_Mime_SimpleMimeEntity::LEVEL_ALTERNATIVE,
            "Content-Type: text/plain\r\n" . "\r\n" . 'foobar'
        );

        $attachment = $this->createChild(
            Swift_Mime_SimpleMimeEntity::LEVEL_MIXED,
    ...
    public function testEntityIsWrittenToByteStream()
    {
        ...
        $is->expects($this->atLeastOnce())->method('write');
    ...
    public function testEntityHeadersAreComittedToByteStream()
    {
        ...
        $is = $this->createInputStream(false);
        $is->expects($this->atLeastOnce())->method('write');
        $is->expects($this->atLeastOnce())->method('commit');
    ...
    {
        $htmlChild = new MimeEntityFixture(
            Swift_Mime_SimpleMimeEntity::LEVEL_ALTERNATIVE,
            "Content-Type: text/html\r\n" . "\r\n" . 'HTML PART',
            'text/html'
        );
        $textChild = new MimeEntityFixture(
            Swift_Mime_SimpleMimeEntity::LEVEL_ALTERNATIVE,
            "Content-Type: text/plain\r\n" . "\r\n" . 'TEXT PART',
            'text/plain'
        );
    ...
    {
        $firstChild = new MimeEntityFixture(
            Swift_Mime_SimpleMimeEntity::LEVEL_ALTERNATIVE,
            "Content-Type: text/plain\r\n" . "\r\n" . 'PART 1',
            'text/plain'
        );
        $secondChild = new MimeEntityFixture(
            Swift_Mime_SimpleMimeEntity::LEVEL_ALTERNATIVE,
            "Content-Type: text/plain\r\n" . "\r\n" . 'PART 2',
            'text/plain'
        );
    ...
    public function testBodyIsReadFromCacheWhenUsingToStringIfPresent()
    {
        ...
        $entity = $this->createEntity($headers, $this->createEncoder(), $cache);
    ...
    public function testBodyIsNotClearedFromCacheIfSameBodySet()
    {
        ...
        // We set the expectation at this point because we only care what happens when calling setBody()
        $cache->shouldReceive('clearKey')->never();
    ...
    public function testBodyIsReadFromCacheWhenUsingToByteStreamIfPresent()
    {
        ...
        $cache
            ...
            ->andReturn(true);
        $cache
            ->shouldReceive('exportToByteStream')
            ->once()
    ...
    public function testBodyIsAddedToCacheWhenUsingToByteStream()
    {
        ...
        $cache
            ...
            ->andReturn(false);
        $cache
            ->shouldReceive('getInputByteStream')
            ->once()
            ->with(\Mockery::any(), 'body');
    ...
    {
        $child = $this->getMockery(
            'Swift_Mime_SimpleMimeEntity'
        )->shouldIgnoreMissing();
        if (isset($level)) {
            $child
                ->shouldReceive('getNestingLevel')
                ->zeroOrMoreTimes()
                ->andReturn($level);
        }
        $child
    ...
    protected function createHeader(
        ...
        $header = $this->getMockery(
            'Swift_Mime_Headers_ParameterizedHeader'
        )->shouldIgnoreMissing();
        $header
            ->shouldReceive('getFieldName')
            ->zeroOrMoreTimes()
            ->andReturn($name);
        $header
            ->shouldReceive('getFieldBodyModel')
            ->zeroOrMoreTimes()
            ->andReturn($model);
        $header
