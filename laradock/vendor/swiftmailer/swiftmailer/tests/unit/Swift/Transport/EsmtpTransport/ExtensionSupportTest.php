...
class Swift_Transport_EsmtpTransport_ExtensionSupportTest extends
    Swift_Transport_EsmtpTransportTest
{
    public function testExtensionHandlersAreSortedAsNeeded()
    {
        ...
        $ext1 = $this->getMockery(
            'Swift_Transport_EsmtpHandler'
        )->shouldIgnoreMissing();
        $ext2 = $this->getMockery(
            'Swift_Transport_EsmtpHandler'
        )->shouldIgnoreMissing();

        $ext1
            ->shouldReceive('getHandledKeyword')
            ->zeroOrMoreTimes()
            ->andReturn('AUTH');
        $ext1
            ->shouldReceive('getPriorityOver')
            ->zeroOrMoreTimes()
            ->with('STARTTLS')
            ->andReturn(1);
        $ext2
    ...
    public function testHandlersAreNotifiedOfParams()
    {
        ...
        $ext1
            ...
            ->andReturn('AUTH');
        $ext1
            ->shouldReceive('setKeywordParams')
            ->once()
            ->with(['PLAIN', 'LOGIN']);
        $ext2
            ->shouldReceive('getHandledKeyword')
    ...
    public function testExtensionsCanModifyMailFromParams()
    {
        ...
        $ext3
            ...
            ->zeroOrMoreTimes()
            ->andReturn('STARTTLS');
        $ext3->shouldReceive('getMailParams')->never();
        $ext3
            ->shouldReceive('getPriorityOver')
            ->zeroOrMoreTimes()
            ->with('AUTH')
    ...
    public function testExtensionsCanExposeMixinMethods()
    {
        ...
        $ext1
            ...
            ->andReturn('AUTH');
        $ext1
            ->shouldReceive('exposeMixinMethods')
            ->zeroOrMoreTimes()
