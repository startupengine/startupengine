 ...
 * @author Chris Corbyn
 */
class Swift_Mime_Headers_ParameterizedHeader extends
    Swift_Mime_Headers_UnstructuredHeader
{
    ...
    {
        $this->setParameters(
            array_merge($this->getParameters(), [$parameter => $value])
        );
    }
    ...
    private function createParameter($name, $value)
    {
        ...
        $maxValueLength =
            $this->getMaxLineLength() - strlen($name . '=*N"";') - 1;
        $firstLineOffset = 0;
        ...
        $valueLines = isset($this->paramEncoder)
            ? explode("\r\n", $value)
            : [$value];
        ...
        if (count($valueLines) > 1) {
            ...
            return $name .
                $this->getEndOfParameterValue($valueLines[0], $encoded, true);
        }
    }
     ...
     * @return string
     */
    private function getEndOfParameterValue(
        $value,
        $encoded = false,
        $firstLine = false
    ) {
