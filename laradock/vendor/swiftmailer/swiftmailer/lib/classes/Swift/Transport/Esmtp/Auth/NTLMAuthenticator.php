 ...
 */
class Swift_Transport_Esmtp_Auth_NTLMAuthenticator implements
    Swift_Transport_Esmtp_Authenticator
{
    ...
    public function authenticate(
        ...
        try {
            ...
            $response = base64_decode(
                substr(trim($this->sendMessage1($agent)), 4)
            );
    ...
    protected function parseMessage2($response)
    {
        ...
        list(
            ...
            $DNSServerName,
            $terminatorByte
        ) = $this->readSubBlock($targetInfoBlock);
    ...
    protected function sendMessage3(
        ...
        if (!$v2) {
            ...
            // LMv2
            $lmResponse = $this->createLMv2Password(
                $password,
                $username,
                $domain,
                $challenge,
    ...
    protected function createMessage3(
        ...
        $userSec = $this->createSecurityBuffer(
            $username,
            ($domainInfo[0] + $domainInfo[1]) / 2
        );
        $userInfo = $this->readSecurityBuffer(bin2hex($userSec));
        $workSec = $this->createSecurityBuffer(
            $workstation,
            ($userInfo[0] + $userInfo[1]) / 2
        );
        $workInfo = $this->readSecurityBuffer(bin2hex($workSec));
        $lmSec = $this->createSecurityBuffer(
            $lmResponse,
            ($workInfo[0] + $workInfo[1]) / 2,
            true
        );
        $lmInfo = $this->readSecurityBuffer(bin2hex($lmSec));
        $ntlmSec = $this->createSecurityBuffer(
            $ntlmResponse,
            ($lmInfo[0] + $lmInfo[1]) / 2,
            true
        );

        return // TYPE 3 message
            // LM response header
            ...
            self::NTLMSIG .
                ...
                $userSec .
                $workSec .
                $this->createByte('000000009a', 8) .
                $this->createByte('01020000') .
                $this->convertTo16bit($domain) .
                $this->convertTo16bit($username) .
                $this->convertTo16bit($workstation) .
                $lmResponse .
                $ntlmResponse;
    }
    ...
    protected function createLMPassword($password, $challenge)
    {
        ...
        $constantDecrypt = $this->createByte(
            $this->desEncrypt(self::DESCONST, $desKey1) .
                $this->desEncrypt(self::DESCONST, $desKey2),
            21,
            false
        );
        ...
        return $this->desEncrypt($challenge, $desKey1) .
            $this->desEncrypt($challenge, $desKey2) .
            $this->desEncrypt($challenge, $desKey3);
    }
    ...
    protected function createLMv2Password(
        ...
        if (strlen($password) <= 15) {
            ...
            $ntml2Hash = $this->md5Encrypt(
                $ntlmHash,
                $this->convertTo16bit(strtoupper($username) . $domain)
            );

            $lmPass = bin2hex(
                $this->md5Encrypt($ntml2Hash, $challenge . $client) . $client
            );
        }
    ...
    protected function createDesKey($key)
    {
        ...
        for ($i = 1; $i < $len; ++$i) {
            ...
            $v = $this->castToByte(
                (ord($key[$i - 1]) << 7 + 1 - $i) |
                    $this->uRShift(
                        hexdec(
                            dechex(hexdec($high) & 0xf) .
                                dechex(hexdec($low) & 0xf)
                        ),
                        $i
                    )
            );
        ...
        $material[] = str_pad(
            substr(dechex($this->castToByte(ord($key[6]) << 1)), -2),
            2,
            '0'
        );
        ...
        foreach ($material as $k => $v) {
            ...
            if ($needsParity) {
                $material[$k] =
                    dechex(hexdec($high) | 0x0) . dechex(hexdec($low) | 0x1);
            } else {
                $material[$k] =
                    dechex(hexdec($high) & 0xf) . dechex(hexdec($low) & 0xe);
            }
        }
    ...
    protected function createSecurityBuffer($value, $offset, $is16 = false)
    {
        ...
        $length = $this->createByte(
            str_pad(dechex($length), 2, '0', STR_PAD_LEFT),
            2
        );
    ...
    {
        return substr(
            openssl_encrypt($value, 'DES-ECB', $key, \OPENSSL_RAW_DATA),
            0,
            8
        );
    }
    ...
    protected function md4Encrypt($input)
    {
        ...
        return function_exists('hash')
            ? hex2bin(hash('md4', $input))
            : mhash(MHASH_MD4, $input);
    }
    ...
    protected function debug($message)
    {
        ...
        if ('02000000' == $messageId) {
            ...
            list($lmLength, $lmOffset) = $this->readSecurityBuffer(
                $data[$i - 1]
            );

            $data[$i++] = substr($message, 40, 16);
            list($ntmlLength, $ntmlOffset) = $this->readSecurityBuffer(
                $data[$i - 1]
            );

            $data[$i++] = substr($message, 56, 16);
            list($targetLength, $targetOffset) = $this->readSecurityBuffer(
                $data[$i - 1]
            );

            $data[$i++] = substr($message, 72, 16);
            list($userLength, $userOffset) = $this->readSecurityBuffer(
                $data[$i - 1]
            );

            $data[$i++] = substr($message, 88, 16);
            list($workLength, $workOffset) = $this->readSecurityBuffer(
                $data[$i - 1]
            );
