...
abstract class Swift_Transport_AbstractSmtpTransport implements Swift_Transport
{
    ...
    public function start()
    {
        if (!$this->started) {
            if (
                ($evt = $this->eventDispatcher->createTransportChangeEvent(
                    $this
                ))
            ) {
    ...
    public function executeCommand(
        ...
        if (
            ($evt = $this->eventDispatcher->createCommandEvent(
                $this,
                $command,
                $codes
            ))
        ...
        if ($pipeline && $this->pipelining) {
            ...
            while ($this->pipeline) {
                list($command, $seq, $codes, $address) = array_shift(
                    $this->pipeline
                );
    ...
    {
        if (
            ($evt = $this->eventDispatcher->createTransportExceptionEvent(
                $this,
                $e
            ))
        ) {
    ...
    protected function getFullResponse($seq)
    {
        ...
        try {
            ...
            $this->throwException(
                new Swift_TransportException($e->getMessage(), 0, $e)
            );
        }
    ...
    private function doMailTransaction(
        $message,
        $reversePath,
        array $recipients,
