 ...
 * @author Chris Corbyn
 */
class Swift_Transport_SendmailTransport extends
    Swift_Transport_AbstractSmtpTransport
{
    ...
    public function send(
        ...
        if (false !== strpos($command, ' -t')) {
            if (
                ($evt = $this->eventDispatcher->createSendEvent(
                    $this,
                    $message
                ))
            ) {
            ...
            if (false === strpos($command, ' -f')) {
                $command .=
                    ' -f' . escapeshellarg($this->getReversePath($message));
            }

            $buffer->initialize(
                array_merge($this->params, ['command' => $command])
            );
