...
class CronExpressionTest extends TestCase
{
    ...
    public function testFactoryRecognizesTemplates()
    {
        $this->assertSame(
            '0 0 1 1 *',
            CronExpression::factory('@annually')->getExpression()
        );
        $this->assertSame(
            '0 0 1 1 *',
            CronExpression::factory('@yearly')->getExpression()
        );
        $this->assertSame(
            '0 0 * * 0',
            CronExpression::factory('@weekly')->getExpression()
        );
    }
     ...
     */
    public function testParsesCronScheduleWithAnySpaceCharsAsSeparators(
        $schedule,
        array $expected
    ) {
        ...
        $this->assertSame(
            $expected[0],
            $cron->getExpression(CronExpression::MINUTE)
        );
        $this->assertSame(
            $expected[1],
            $cron->getExpression(CronExpression::HOUR)
        );
        $this->assertSame(
            $expected[2],
            $cron->getExpression(CronExpression::DAY)
        );
        $this->assertSame(
            $expected[3],
            $cron->getExpression(CronExpression::MONTH)
        );
        $this->assertSame(
            $expected[4],
            $cron->getExpression(CronExpression::WEEKDAY)
        );
    }
    ...
    public function scheduleProvider()
    {
        return array(
            ...
            array(
                '0 0 2W * *',
                strtotime('2011-07-01 00:00:00'),
                '2011-07-01 00:00:00',
                true
            ),
            array(
                '0 0 1W * *',
                strtotime('2011-05-01 00:00:00'),
                '2011-05-02 00:00:00',
                false
            ),
            array(
            ...
            array(
                '* * * * 1L',
                strtotime('2011-07-24 00:00:00'),
                '2011-07-25 00:00:00',
                false
            ),
            array(
                '* * * 1 5L',
                strtotime('2011-12-25 00:00:00'),
                '2012-01-27 00:00:00',
                false
            ),
            ...
            [
                '* * * * MON-FRI',
                strtotime('2017-01-08 00:00:00'),
                strtotime('2017-01-09 00:00:00'),
                false
            ],
            [
                '* * * * TUE',
                strtotime('2017-01-08 00:00:00'),
                strtotime('2017-01-10 00:00:00'),
                false
            ]
        );
    }
     ...
     */
    public function testDeterminesIfCronIsDue(
        $schedule,
        $relativeTime,
        $nextRun,
        $isDue
    ) {
        $relativeTimeString = is_int($relativeTime)
            ? date('Y-m-d H:i:s', $relativeTime)
            : $relativeTime;
    ...
    public function testIsDueHandlesDifferentDefaultTimezones()
    {
        ...
        $this->assertFalse(
            $cron->isDue(new DateTime($date), 'Europe/Amsterdam')
        );
    ...
    public function testIsDueHandlesDifferentSuppliedTimezones()
    {
        ...
        $this->assertTrue(
            $cron->isDue(new DateTime($date, new DateTimeZone('UTC')), 'UTC')
        );
        $this->assertFalse(
            $cron->isDue(
        ...
        $this->assertFalse(
            $cron->isDue(
                new DateTime($date, new DateTimeZone('Asia/Tokyo')),
                'UTC'
            )
        );
        $this->assertFalse(
    ...
    public function testIsDueHandlesDifferentTimezonesAsArgument()
    {
        ...
        $this->assertFalse(
            $cron->isDue(new DateTime($date, $amsterdam), 'UTC')
        );
        $this->assertFalse($cron->isDue(new DateTime($date, $tokyo), 'UTC'));
        $this->assertFalse(
            $cron->isDue(new DateTime($date, $utc), 'Europe/Amsterdam')
        );
        $this->assertTrue(
            $cron->isDue(new DateTime($date, $amsterdam), 'Europe/Amsterdam')
        );
        $this->assertFalse(
            $cron->isDue(new DateTime($date, $tokyo), 'Europe/Amsterdam')
        );
        $this->assertFalse(
            $cron->isDue(new DateTime($date, $utc), 'Asia/Tokyo')
        );
        $this->assertFalse(
            $cron->isDue(new DateTime($date, $amsterdam), 'Asia/Tokyo')
        );
        $this->assertTrue(
            $cron->isDue(new DateTime($date, $tokyo), 'Asia/Tokyo')
        );
    }
    ...
    public function testRecognisesTimezonesAsPartOfDateTime()
    {
        ...
        $dtCurrent = \DateTime::createFromFormat(
            "!Y-m-d H:i:s",
            "2017-10-17 10:00:00",
            $tzServer
        );
        ...
        $this->assertEquals(
            '1508151600 : 2017-10-16T07:00:00-04:00 : America/New_York',
            $dtPrev->format("U \: c \: e")
        );
        ...
        $dtPrev = $cron->getPreviousRunDate(
            $dtCurrent->format("c"),
            0,
            true,
            $tzCron
    ...
    public function testCanIterateOverNextRuns()
    {
        ...
        $nextRun = $cron->getNextRunDate(
            $cron->getNextRunDate("2008-11-09 00:00:00", 1, true),
            1,
            true
        );
    ...
    public function testSkipsCurrentDateByDefault()
    {
        ...
        $this->assertSame(
            $current->format('Y-m-d H:i:00'),
            $nextPrev->format('Y-m-d H:i:s')
        );
    }
    ...
    public function testStripsForSeconds()
    {
        ...
        $this->assertSame(
            '2011-09-27 10:11:00',
            $cron->getNextRunDate($current)->format('Y-m-d H:i:s')
        );
    }
    ...
    public function testFixesPhpBugInDateIntervalMonth()
    {
        ...
        $this->assertSame(
            '2011-01-27 00:00:00',
            $cron
                ->getPreviousRunDate('2011-08-22 00:00:00')
                ->format('Y-m-d H:i:s')
        );
    }
    ...
    public function testIssue29()
    {
        ...
        $this->assertSame(
            '2013-03-10 00:00:00',
            $cron
                ->getPreviousRunDate('2013-03-17 00:00:00')
                ->format('Y-m-d H:i:s')
        );
    }
    ...
    public function testValidationWorks()
    {
        ...
        $this->assertFalse(
            CronExpression::isValidExpression('990 14 * * mon-fri0345345')
        );

        // see https://github.com/dragonmantank/cron-expression/issues/5
        $this->assertTrue(
            CronExpression::isValidExpression('2,17,35,47 5-7,11-13 * * *')
        );
    }
