...
class Log extends Model
{
    ...
    {
        if ($this->type == 'exception') {
            $occurrences = \App\Log::where(
                'created_at',
                '>=',
                \Carbon\Carbon::now()->subHours($hours)
            )
                ->whereJsonContains('content->class', $this->content['class'])
                ->get();
            ...
            $occurrences = \App\Log::where(
                ...
                ->whereJsonContains('content->key', $this->content['key'])
                ->get();
            ...
            $occurrences = \App\Log::where(
                ...
                ->whereJsonContains('content->uri', $this->content['uri'])
                ->get();
            ...
            $occurrences = \App\Log::where(
                ...
                ->whereJsonContains('content->uri', $this->content['sql'])
                ->get();
        } else {
            $occurrences = null;
        }
