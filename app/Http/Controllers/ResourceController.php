...
class ResourceController extends Controller
{
                ...
                if ($request->input('save') == true) {
                    foreach ($newArray as $field => $data) {
                        if (strpos($data[0], '.') !== false) {
                            $fieldString = convertDotsToArrows(
                                "json." . $data[0]
                            );
                        } else {
                            $fieldString = $data[0];
                    ...
                    if (isset($jsonInput['slug'])) {
                        $item->slug = createSlug(
                            $request->input('data')['slug']
                        );
                        //dd($item->slug);
                    }
    ...
    public function count(Request $request)
    {
        ...
        $query = DB::select(
            DB::raw(
                'SELECT date(created_at) as CREATED, count(id) as TOTAL FROM ' .
                    $table .
