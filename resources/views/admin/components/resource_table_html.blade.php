...
<div v-else
     style="@if(isset($options['CONTAINER_STYLE'])) {{ $options['CONTAINER_STYLE'] }} @else width:calc(100%);opacity:0;height:100%; @endif"
    ...
    <div class="{{ $options['WRAPPER_CLASS'] }}"
        ...
        <div class="card card-small mb-5" id="listView" style="min-width:100%;height:100%;"
            ...
            <div class="card-body p-0 pb-0 text-center">
                <table class="table mb-0">
                    ...
                    <tr class="dataRow" v-for="(item, index) in info.data"
                        v-if="info != null && info.data != null && info.data.length > 0"
                    ...
                    @if( isset($options['TABLE_ROW_NO_RESULTS_CONDITIONS'] ) && !isset($options['TABLE_ROW_NO_RESULTS_MESSAGE']))
                        <td v-if="info != null && info.data != null && info.meta != null && info.meta.total != null && info.meta.total > 0 && info.data.every(function(item){return ( {!! $options['TABLE_ROW_NO_RESULTS_CONDITIONS'] !!} )}) && status != 'loading'"
                            ...
                            style="height:55px;">
                            No results. Try different settings.
                        </td>
                    @endif
                    ...
                    @if(isset($options['TABLE_ROW_NO_RESULTS_MESSAGE']))
                        <tr class="dataRow"
                            v-if="info.data.every(function(item){return ( {!! $options['TABLE_ROW_NO_RESULTS_CONDITIONS'] !!} )}) == false">
            ...
            @else
                ...
                <div align="center"
                     v-if="info != null && info.meta != null && info.meta.pages != null && info.meta.pages > 0 && status != 'loading'"
                     class="card-footer align-text-bottom"
    ...
    'CARD_HEADER_FIELD'
                    ...
                    @if(isset($options['SHOW_PAGINATION']) && $options['SHOW_PAGINATION'] != false)
                        <div class="col-md-12 pb-4 px-0" align="center"
                             v-if="info.meta.pages > 0 && status != 'loading'">
                            <nav aria-label="Pagination" class="pull-left page-meta-container"
                                 style="display:inline-block;float:left; width:75px;text-align:left;">
