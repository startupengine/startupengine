...
@section('content')
    ...
    <?php $header = '<div class="btn-group">
                            ...
                            <span class="btn btn-light border" v-bind:class="{ active: filters.status==\'status=ACTIVE\'}"  v-on:click="updateFilters({\'status\':\'status=ACTIVE\'})">
                                <span>Active</span>
                            </span>
                            <span class="btn btn-light border" v-bind:class="{ active: filters.status==\'status=INACTIVE\'}"   v-on:click="updateFilters({\'status\':\'status=INACTIVE\'})">
                                <span>Inactive</span>
                            </span>
                        </div>'; ?>
...
@section('scripts')
    {!! renderResourceTableScriptsDynamically(['url' =>  URL::to('/').'/api/resources/page', 'LIMIT' => 1000, 'FILTERS' => "{'status':'status=ACTIVE'}"]) !!}
@endsection