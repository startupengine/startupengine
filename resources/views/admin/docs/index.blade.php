...
@section('page-title') Docs @endsection
...
@section('content')
    ...
    <?php $header = '<div class="btn-group">
        ...
        <?php $tableHeader = '<th scope="col" class="border-0" style="width:50px !important;text-align:left;">Image</th><th scope="col" class="border-0 hiddenOnMobile" style="width:100px !important;text-align:left;">Status</th><th scope="col" class="border-0 " style="text-align:left;">File Path</th><th scope="col" class="border-0">&nbsp;</th>'; ?>
        <?php $tableRow = '<td align="left" style="width:30px;"><span class="thumbnail" v-if="item.thumbnail != null" v-bind:style="{ backgroundImage: \'url(\' + item.thumbnail + \')\' }"> </span><span class="thumbnail" v-else></span></td><td align="left" style="width:100px;" class="hiddenOnMobile"><span class="badge badge-type text-dark mr-2" style="width:100%;" >{{ item.status }}</span></td><td style="width:auto;" align="left"><span class="badge text-dark mr-2 px-0 truncate-100 text-left">{{ item.path }}</span></td><td align="center"  style="width:50px;vertical-align:middle;"><a href="#" class="btn btn-white" v-bind:href="\'/admin/docs/\' + item.id" class="btn btn-white" style="border-radius:15px;margin-right:5px;width:30px;height:30px;padding:7px 5px 6px 7px;"><i class="fa fa-fw fa-angle-right"></i></a></td>'; ?>
        ...
        {!! renderResourceTableHtmlDynamically(['HEADER' => $header, 'TABLE_HEADER' => $tableHeader, 'TABLE_ROW' => $tableRow, 'PATH' => '/admin/content', 'CARD_HEADER_FIELD' => 'name', 'CARD_BODY_FIELD' => 'excerpt', 'CARD_BODY_HTML' => $cardHtml, 'CARD_FOOTER' => $cardFooter]) !!}
...
@section('scripts')
    {!! renderResourceTableScriptsDynamically(['url' => URL::to('/').'/api/resources/doc', 'FILTERS' => "{status: 'status=PUBLISHED'}", 'VUE_APP_NAME' => 'contentApp', 'LIMIT' => 1000]) !!}
@endsection