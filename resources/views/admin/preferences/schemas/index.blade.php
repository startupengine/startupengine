...
@section('title') Preference Schemas - <?php echo setting('site.title'); ?> @endsection
...
@section('page-title') Preference Schemas @endsection
...
@section('content')
    ...
    <?php $header = 'List of all preference schemas.'; ?>
        <?php $tableHeader = '<th scope="col" class="border-0 hiddenOnMobile" style="width:100px !important;text-align:left;">Key</th><th scope="col" class="border-0 " style="text-align:left;">Description</th><th scope="col" class="border-0">&nbsp;</th>'; ?>
        <?php $tableRow = '<td align="left" style="width:100px;" class="hiddenOnMobile"><span class="badge badge-type text-dark mr-2" style="width:100%;" >{{ item.name }}</span></td><td style="width:auto;" align="left"><span class="badge text-dark mr-2 px-0">{{ item.description }}</span></td><td align="center"  style="width:50px;vertical-align:middle;"><a href="#" class="btn btn-white" v-bind:href="\'/admin/preferences/schemas/\' + item.id" class="btn btn-white" style="border-radius:15px;margin-right:5px;width:30px;height:30px;padding:7px 5px 6px 7px;"><i class="fa fa-fw fa-angle-right"></i></a></td>'; ?>
        {!! renderResourceTableHtml(['HEADER' => $header, 'TABLE_HEADER' => $tableHeader, 'TABLE_ROW' => $tableRow, 'PATH' => '/admin/content/']) !!}
...
@section('scripts')
    {!! renderResourceTableScriptsDynamically(['url' =>  URL::to('/').'/api/resources/preferenceschema', 'LIMIT' => 1000]) !!}
@endsection